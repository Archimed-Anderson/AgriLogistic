...
import { Sprout, ArrowRight, Plus, History, LayoutDashboard } from 'lucide-react';
...
const Parcel3DViewer = dynamic(() => import('./Parcel3DViewer'), {
  ssr: false,
  loading: () => (
    <div className="h-full w-full flex items-center justify-center bg-black/5 text-muted-foreground animate-pulse">
...
export function FarmerDashboard() {
  const { activeTab, setParcels, setWeather } = useFarmerStore();
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              <div>
                ...
                <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.2em] mt-2 flex items-center gap-2">
                  <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /> Système de
                  Surveillance Prédictive Actif
                </p>
              </div>
              <div className="flex gap-4">
                ...
                <Button
                  variant="outline"
                  className="rounded-2xl h-14 px-6 font-black border-slate-200 hover:bg-white dark:hover:bg-white/5 transition-all"
                >
                ...
                <CardHeader className="flex flex-row items-center justify-between p-8 border-b border-slate-50 dark:border-white/5">
                  <div>
                    <CardTitle className="text-2xl font-black text-[#1B4D3E] dark:text-white flex items-center gap-3">
                      <LayoutDashboard className="h-6 w-6 text-[#D4A017]" /> Jumeau Numérique Temps
                      Réel
                    </CardTitle>
                    <CardDescription className="font-medium">
                      Analyse multispectrale par satellite augmentée IA
                    </CardDescription>
                  </div>
                  <div className="flex gap-2">
                    <Badge className="bg-emerald-500/10 text-emerald-600 border-none px-4 py-1.5 rounded-full font-black text-[10px]">
                      SYNC: MQTT/SSL
                    </Badge>
                    <Badge className="bg-[#D4A017]/10 text-[#D4A017] border-none px-4 py-1.5 rounded-full font-black text-[10px]">
                      MODE: 3D_VR
                    </Badge>
                  </div>
                </CardHeader>
