  ...
  const [tests, setTests] = useState<DiagnosticTest[]>([
    {
      ...
      description: 'Vérifie la latence vers le gateway API principal.',
      icon: Globe,
      status: 'idle',
    },
    {
      ...
      description: "Analyse l'état des réplicas PostgreSQL.",
      icon: Database,
      status: 'idle',
    },
    {
      id: 'disk',
      name: 'Scan S.M.A.R.T',
      description: "Vérifie l'état de santé physique des disques.",
      icon: Zap,
      status: 'idle',
    },
    {
      id: 'auth',
      name: 'Sync Auth Service',
      description: 'Valide la synchronisation des jetons JWT.',
      icon: ShieldCheck,
      status: 'idle',
    },
  ]);
    ...
    <div className="bg-card/40 backdrop-blur-xl border border-border rounded-[32px] overflow-hidden flex flex-col h-full shadow-2xl transition-all hover:bg-card/50">
      ...
      <div className="flex-1 p-6 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-foreground/10">
        {tests.map((test) => (
          <div
            key={test.id}
            className="p-4 rounded-3xl bg-foreground/5 border border-border group transition-all hover:border-primary/30"
          >
                ...
                <div
                  className={cn(
                    ...
                    test.status === 'success'
                      ...
                      : 'bg-background border-border text-muted-foreground group-hover:text-primary group-hover:border-primary/20'
                  )}
                >
                  <test.icon className="w-4 h-4" />
                </div>
                <div>
                  <h4 className="text-[11px] font-black text-foreground uppercase tracking-tight">
                    {test.name}
                  </h4>
                  <p className="text-[9px] text-muted-foreground font-medium leading-[1.2] mt-0.5">
                    {test.description}
                  </p>
                </div>
              </div>
                ...
                >
                  <div className="flex items-center justify-between text-[8px] font-black uppercase tracking-widest tabular-nums">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span
                        className={test.status === 'success' ? 'text-emerald-500' : 'text-rose-500'}
                      >
                        {test.result}
                      </span>
                    </div>
      ...
      <div className="p-4 bg-foreground/5 border-t border-border text-center">
        <p className="text-[8px] font-black text-muted-foreground opacity-40 uppercase tracking-[0.3em]">
          Module de Diagnostic v4.2
        </p>
      </div>
    </div>
  );
