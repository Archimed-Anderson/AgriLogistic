...
export class OrderController {
  ...
  /**
   * Get sales statistics for a product (Internal use by AI service)
   */
    ...
    try {
      const result = await Database.query(
        `SELECT DATE(o.created_at) as date, SUM(oi.quantity) as total_quantity
         ...
         WHERE oi.product_id = $1 AND o.status NOT IN ('cancelled', 'refunded', 'pending')
      ...
      console.error('[OrderController] Get sales stats error:', error);
      res.status(500).json({
        success: false,
