...
import { History, Search, ShieldCheck, UserCircle, Clock, ArrowRight, Filter } from 'lucide-react';
...
const MOCK_LOGS: AuditLog[] = [
  {
    ...
    targetUserName: 'Sophie Ndiaye',
    timestamp: '2024-01-24T18:45:00Z',
    details: "Début d'impersonation : Vérification erreur panier",
  },
  {
    id: 'LOG-002',
    adminId: 'ADMIN-001',
    ...
    targetUserName: 'Admin Secondary',
    timestamp: '2024-01-24T18:30:00Z',
    details: 'Ajout permission [Accès CLI]',
  },
  {
    id: 'LOG-003',
    adminId: 'ADMIN-002',
    adminName: 'Admin Tech',
    action: 'USER_SUSPEND',
    targetUserId: 'USR-005',
    targetUserName: 'Abdou Konate',
    timestamp: '2024-01-23T14:20:00Z',
    details: 'Suspension : Activité suspecte détectée',
  },
  {
    id: 'LOG-004',
    adminId: 'ADMIN-001',
    ...
    <div className="flex flex-col h-full bg-card/40 backdrop-blur-xl rounded-[40px] overflow-hidden border border-border shadow-2xl">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">
                Traçabilité Immutable • {MOCK_LOGS.length} Entrées
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button
              variant="ghost"
              size="icon"
              className="w-9 h-9 rounded-xl hover:bg-foreground/5"
            >
              <Search className="w-4 h-4" />
            </Button>
            <Button
              variant="outline"
              className="rounded-xl border-border bg-background/50 backdrop-blur-md h-10 gap-2 shadow-sm text-[10px] font-black uppercase tracking-widest"
            >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-foreground/10 p-6 space-y-3">
        {MOCK_LOGS.map((log) => (
          <div
            key={log.id}
            className="group p-5 rounded-[32px] bg-foreground/5 border border-border transition-all hover:bg-card hover:shadow-xl hover:border-amber-500/20 flex flex-col md:flex-row md:items-center gap-6"
          >
            ...
            <div className="md:w-1/3 flex items-center justify-between border-t md:border-t-0 md:border-l border-border/50 pt-4 md:pt-0 md:pl-6">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="shrink-0 w-8 h-8 opacity-0 group-hover:opacity-100 transition-all"
              >
      ...
      <div className="p-6 bg-amber-500/5 border-t border-amber-500/20 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <span className="text-[9px] font-black text-muted-foreground uppercase tracking-widest leading-none">
            Journal Immutable SHA-256 Verifié
          </span>
        </div>
        <p className="text-[9px] font-mono font-bold text-amber-500 opacity-60">
          SESS-BLOCK: x9k2-p9L0-v4M1
        </p>
      </div>
    </div>
  );
