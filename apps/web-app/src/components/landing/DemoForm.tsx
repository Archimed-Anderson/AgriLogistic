...
const demoSchema = z.object({
  ...
  email: z.string().email("Email invalide").refine(
    (email) => {
      const forbidden = ["gmail.com", "outlook.com", "hotmail.com", "yahoo.com", "live.com", "icloud.com", "aol.com"]
      ...
      <div className="flex flex-col items-center justify-center p-8 md:p-14 text-center space-y-8 animate-in fade-in zoom-in duration-700 bg-white rounded-[40px] shadow-2xl border border-emerald-50">
        ...
        <div className="space-y-3">
          ...
          <p className="max-w-[320px] text-slate-500 font-medium leading-relaxed">
            Nous avons bien enregistré votre demande. Un expert AgriLogistic vous contactera sous <span className="text-primary font-bold">24h</span>.
          </p>
        </div>
        <div className="pt-4">
          <Button 
    ...
    <div className="w-full max-w-lg mx-auto bg-white p-8 md:p-12 rounded-[40px] shadow-[0_32px_80px_-20px_rgba(0,0,0,0.08)] border border-slate-100/50 animate-fade-in-up relative overflow-hidden">
      ...
      <div className="mb-10 space-y-2">
        <h2 className="text-3xl font-black text-[#0A2619] tracking-tight leading-tight">
          L'intelligence AgriLogistic,<br/>
        ...
        <p className="text-sm font-medium text-slate-500 leading-relaxed max-w-[90%]">
          Rejoignez l'élite technologique agricole. Nos experts configurent votre environnement de test sous 24h.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-5">
          <div className="space-y-2">
            <Label htmlFor="firstName" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Prénom</Label>
            <Input
              id="firstName"
              placeholder="Prénom"
              className={cn(
              ...
              {...register("firstName")}
            />
            {errors.firstName && <p className="text-[10px] text-red-500 font-bold mt-1.5 ml-1">{errors.firstName.message}</p>}
          </div>
          <div className="space-y-2">
            <Label htmlFor="lastName" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Nom</Label>
            <Input
              id="lastName"
              placeholder="Nom"
              className={cn(
              ...
              {...register("lastName")}
            />
            {errors.lastName && <p className="text-[10px] text-red-500 font-bold mt-1.5 ml-1">{errors.lastName.message}</p>}
          </div>
        </div>

        <div className="space-y-2">
          <Label htmlFor="email" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Email Professionnel</Label>
          <Input
            id="email"
            type="email"
        ...
        <div className="space-y-2">
          <Label htmlFor="company" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Organisation / Structure</Label>
          <Input
            id="company"
            ...
            {...register("company")}
          />
          {errors.company && <p className="text-[10px] text-red-500 font-bold mt-1.5 ml-1">{errors.company.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="actorType" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Secteur d'activité</Label>
          <div className="relative">
            <select
              id="actorType"
              className={cn(
              ...
              {...register("actorType")}
            >
              <option value="" disabled>Sélectionner votre secteur</option>
          ...
          {errors.actorType && <p className="text-[10px] text-red-500 font-bold mt-1.5 ml-1">{errors.actorType.message}</p>}
        </div>

        <div className="space-y-2">
          <Label htmlFor="message" className="text-[10px] font-black uppercase text-slate-400 tracking-[0.1em] ml-1">Votre Objectif</Label>
          <textarea
            id="message"
            rows={3}
            ...
            {...register("message")}
          />
          {errors.message && <p className="text-[10px] text-red-500 font-bold mt-1.5 ml-1">{errors.message.message}</p>}
        </div>
        ...
        <div className="pt-6 flex items-center justify-center gap-8 opacity-40 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500">
           <p className="text-[9px] font-black uppercase tracking-widest text-[#0A2619]">RGPD COMPLIANT</p>
           <div className="h-3 w-px bg-slate-300" />
           <p className="text-[9px] font-black uppercase tracking-widest text-[#0A2619]">SSL SECURED</p>
           <div className="h-3 w-px bg-slate-300" />
           <p className="text-[9px] font-black uppercase tracking-widest text-[#0A2619]">DATA PRIVACY</p>
        </div>
      </form>
    </div>
  )
