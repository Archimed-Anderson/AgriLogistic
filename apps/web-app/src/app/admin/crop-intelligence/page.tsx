...
import { cropZonesData, getCropStatistics, getHealthStatus, type CropZone } from '@/data/crop-intelligence-data'
...
export default function CropIntelligencePage() {
  ...
  const getWeatherIcon = (condition: string) => {
    switch (condition) {
      case 'Ensoleillé': return <Sun className="h-5 w-5 text-yellow-500" />
      case 'Nuageux': return <Cloud className="h-5 w-5 text-slate-400" />
      case 'Pluvieux': return <CloudRain className="h-5 w-5 text-blue-500" />
      case 'Sec': return <Sun className="h-5 w-5 text-orange-500" />
    ...
    <div className="min-h-screen bg-linear-to-br from-slate-50 via-white to-emerald-50/20 p-6">
        ...
        <div className="flex items-start justify-between gap-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-slate-600 font-medium">Surveillance agricole par IA et données satellites</p>
            </div>
          </div>
          ...
          >
            <Satellite className={`h-5 w-5 ${isScanning ? 'animate-spin' : 'group-hover:rotate-12 transition-transform'}`} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-200 hover:border-blue-300 hover:shadow-2xl transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Zones Surveillées</p>
        ...
        {/* Alertes Actives */}
        <div className={`bg-white rounded-3xl p-8 border transition-all duration-300 ${
          activeAlerts > 0 
          ...
          <div>
            <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Alertes Actives</p>
            <p className={`text-5xl font-black mb-1 ${
              activeAlerts > 0 ? 'text-red-600' : 'text-emerald-600'
            }`}>{activeAlerts}</p>
            <p className="text-xs text-slate-500 font-medium">
              {activeAlerts > 0 ? `${activeAlerts} zone(s) nécessite(nt) attention` : 'Toutes les zones sont saines'}
            </p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-3xl p-8 border border-slate-200 hover:border-orange-300 hover:shadow-2xl transition-all duration-300">
          ...
          <div>
            <p className="text-sm font-bold text-slate-600 mb-2 uppercase tracking-wide">Rendement Moyen Prévu</p>
            ...
            <p className="text-xs text-slate-500 font-medium">Total: {(stats.totalEstimatedYield / 1000).toFixed(1)}k tonnes</p>
          </div>
        </div>
      </div>
            ...
            >
              ...
              <div className="relative h-56 overflow-hidden">
                ...
                <div className="absolute top-4 right-4">
                  <div className={`px-4 py-2 rounded-full backdrop-blur-xl bg-white/90 border ${healthStatus.color.split(' ')[1]} shadow-xl`}>
              ...
              <div className="p-6">
                ...
                {/* Irrigation Status */}
                <div className={`px-3 py-2 rounded-xl border flex items-center gap-2 ${getIrrigationBadge(zone.irrigationStatus)}`}>
                ...
                <div className="mt-4 pt-4 border-t border-slate-100 flex items-center gap-2 text-sm text-slate-500">
                  ...
                  <span>Scan: {new Date(zone.lastScan).toLocaleString('fr-FR', { 
                    day: '2-digit', 
                    month: 'short', 
                    hour: '2-digit', 
                    minute: '2-digit' 
