...
export function AlertsView() {
  ...
  const AlertCard = ({ alert }: { alert: IoTAlert }) => {
    ...
    return (
      <div
        className={`relative flex gap-4 p-4 rounded-r-xl border shadow-sm transition-all ${borderClass} ${bgClass}`}
      >
        ...
        {/* Icon */}
        <div
          className={`flex items-center justify-center w-12 h-12 rounded-xl shrink-0 ${iconColorClass}`}
        >
        ...
        <div className="flex-1">
          <div className="flex justify-between items-start">
            <div>
              <h4
                className={`font-bold text-base ${isResolved ? 'line-through text-muted-foreground' : ''}`}
              >
                {alert.type} Alert{' '}
                <span className="text-xs font-normal text-muted-foreground ml-2">#{alert.id}</span>
              </h4>
            ...
            <div className="text-right">
              {isResolved ? (
                ...
                <span
                  className={`inline-flex items-center text-xs font-bold px-2 py-1 rounded-full uppercase tracking-wider ${
      ...
      <div className="relative pl-6 space-y-4 border-l-2 border-slate-200 dark:border-slate-800 ml-4 pb-10">
        {sortedAlerts.map((alert) => (
          <div key={alert.id} className="relative">
            {/* Timeline Dot */}
            <div
              className={`absolute -left-[31px] top-6 w-4 h-4 rounded-full border-2 border-white dark:border-slate-900 ${
