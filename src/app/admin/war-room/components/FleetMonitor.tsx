    ...
    <div className="flex flex-col h-full bg-card/40 backdrop-blur-xl rounded-[40px] overflow-hidden border border-border shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-sm font-black text-foreground uppercase tracking-tighter">
                Répertoire de la Flotte
              </h3>
              <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">
                Temps Réel • {vehicles.length} Véhicules
              </p>
            </div>
          </div>
        ...
        <table className="w-full text-left border-separate border-spacing-y-2">
          <thead>
            <tr className="text-[9px] font-black text-muted-foreground uppercase tracking-[0.2em] px-4">
              <th
                className="pb-4 pl-4 cursor-pointer hover:text-primary transition-colors"
                onClick={() => toggleSort('id')}
              >
                Véhicule <ArrowUpDown className="inline w-3 h-3 ml-1" />
              </th>
              <th
                className="pb-4 cursor-pointer hover:text-primary transition-colors"
                onClick={() => toggleSort('status')}
              >
                Statut <ArrowUpDown className="inline w-3 h-3 ml-1" />
              </th>
          ...
          <tbody>
            {filteredVehicles.map((vehicle) => (
              <tr
                key={vehicle.id}
                className="group hover:bg-foreground/5 transition-all duration-300 rounded-[20px]"
              >
                ...
                <td className="py-4">
                  <div className="flex items-center gap-4">
                    ...
                    <MetricMini
                      icon={Fuel}
                      value={`${vehicle.fuel}%`}
                      color={vehicle.fuel < 30 ? 'text-rose-500' : 'text-emerald-500'}
                    />
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-1.5 mt-0.5">
                      ...
                      <p className="text-[10px] font-bold text-muted-foreground">
                        {vehicle.destination}{' '}
                        <span className="text-primary ml-1">(ETA: {vehicle.eta})</span>
                      </p>
                    </div>
                  </div>
                </td>
                <td className="py-4 text-right pr-4 rounded-r-[20px]">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="w-8 h-8 rounded-lg hover:bg-foreground/10 text-muted-foreground"
                    >
      ...
      <div className="p-6 bg-foreground/5 border-t border-border flex items-center justify-between">
        <p className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.2em]">
          {filteredVehicles.length} Véhicules Affichés
        </p>
        <Button
          variant="ghost"
          className="text-[9px] font-black text-primary uppercase tracking-widest gap-2"
        >
    ...
    >
      <div
        className={cn(
          ...
          status === 'active' ? 'animate-pulse bg-emerald-500' : 'bg-current opacity-40'
        )}
      />
      {status}
    </div>
...
function MetricMini({ icon: Icon, value, color }: { icon: any; value: string; color: string }) {
  return (
    <div className="flex items-center gap-1.5 group/metric">
      <Icon
        className={cn(
          'w-3.5 h-3.5 opacity-40 group-hover/metric:opacity-100 transition-opacity',
          color
        )}
      />
