    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      ...
      <div className="bg-card p-4 rounded-xl border shadow-sm flex justify-between items-center">
        ...
        <div className="flex gap-2">
            <span className="text-xs font-mono bg-muted px-2 py-1 rounded">Dispo: {trucks.filter(t => t.status === 'Available').length}</span>
            <span className="text-xs font-mono bg-muted px-2 py-1 rounded">En Route: {trucks.filter(t => t.status === 'In Transit' || t.status === 'Assigned').length}</span>
        </div>
      </div>
             ...
             {trucks.map((truck) => (
               <TableRow key={truck.id}>
                 ...
                 <TableCell>
                    ...
                    <div className="text-xs flex items-center gap-1 text-yellow-600">★ {truck.driverRating}</div>
                 </TableCell>
                 <TableCell className="text-center">
                    <span className="bg-slate-100 dark:bg-slate-800 font-bold px-2 py-1 rounded text-xs">{truck.capacity}T</span>
                 </TableCell>
                 <TableCell>
