...
export function MetricDrillDown({ isOpen, onClose, title, metricValue, color, children }: DrillDownProps) {
  return (
    <AnimatePresence>
      {isOpen && (
        <React.Fragment>
          ...
          >
            ...
            <div className="p-8 border-b border-border bg-foreground/5 flex items-center justify-between">
              <div className="flex items-center gap-4">
                 <div className={cn("p-3 rounded-2xl border", color.replace('text-', 'bg-').replace('-500', '-500/10 border-').concat('-500/20').concat(' ').concat(color))}>
                    <TrendingUp className="w-6 h-6" />
                 </div>
                 <div>
                    <h2 className="text-2xl font-black tracking-tighter text-foreground uppercase">{title}</h2>
                    <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.2em]">Analyse Granulaire & Historique</p>
                 </div>
              </div>
              <button 
                onClick={onClose}
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 scrollbar-thin scrollbar-thumb-foreground/10">
               ...
               <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <StatItem label="Valeur Actuelle" value={metricValue} color={color} icon={Activity} />
                  <StatItem label="Moyenne (1h)" value="14.2%" color="text-muted-foreground" icon={Clock} />
                  <StatItem label="Seuil d'Alerte" value="85%" color="text-rose-500" icon={AlertCircle} />
               </div>
               ...
               <div className="bg-foreground/5 border border-border rounded-[32px] p-8 h-[400px] border-dashed flex flex-col items-center justify-center">
                  ...
                  <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground">Graphique Haute Résolution en cours de chargement...</p>
               </div>
            ...
            <div className="p-6 bg-foreground/5 border-t border-border flex items-center justify-between">
               <div className="flex items-center gap-2">
                 ...
                 <span className="text-[8px] font-black text-muted-foreground uppercase tracking-widest">Flux Synchronisé: OK</span>
               </div>
