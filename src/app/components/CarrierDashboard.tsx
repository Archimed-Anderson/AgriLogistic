...
export function CarrierDashboard() {
  ...
  const [selectedTab, setSelectedTab] = useState<
    'missions' | 'route' | 'documents' | 'stats' | 'fleet'
  >('missions');
  ...
  const fleetStats = {
    ...
    maintenanceNeeded: vehicles.filter(
      (v) =>
    ...
    avgEfficiency: (vehicles.reduce((acc, v) => acc + v.efficiency, 0) / vehicles.length).toFixed(
      1
    ),
            ...
            <div className="flex gap-4">
              {[
                { id: 'missions', label: 'Missions', icon: Package, count: stats.activeMissions },
                {
                  ...
                  icon: Truck,
                  count: fleetStats.maintenanceNeeded,
                },
