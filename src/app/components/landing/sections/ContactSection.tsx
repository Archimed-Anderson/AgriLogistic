            ...
            <div className="lg:col-span-3 p-8 lg:p-12 xl:p-16 flex flex-col justify-center">
                ...
                {contactOptions.map((option) => (
                  <div
                    ...
                    className={`p-4 rounded-xl border cursor-pointer transition-all ${
                      activeContact.id === option.id
                        ? 'border-green-500 bg-green-50/50 ring-1 ring-green-500'
                        : 'border-slate-200 hover:border-green-200 hover:bg-white'
                    }`}
                    onClick={() => setActiveContact(option)}
                    onDoubleClick={() => onNavigate && onNavigate(option.route)}
                  >
                    <div
                      className={`w-10 h-10 ${option.color} rounded-lg flex items-center justify-center mb-3`}
                    >
                    ...
                    <p className="font-semibold text-sm text-slate-900 flex items-center gap-1">
                      ...
                      <ArrowRight
                        className={`w-3 h-3 ${
                          activeContact.id === option.id ? 'opacity-100' : 'opacity-0'
                        } transition-opacity`}
                      />
                    </p>
                  </div>
              ...
              <form className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    placeholder={`Reg: ${activeContact.title}`}
                    className="bg-white border-slate-200 h-11"
                    readOnly
                  />
                </div>
