...
const NODES = [
  ...
  { id: "payment", label: "Escrow Payment", icon: DollarSign, x: 20, y: 80, color: "text-emerald-400" },
  { id: "delivery", label: "GPS Trigger", icon: Truck, x: 80, y: 80, color: "text-blue-400" },
  { id: "quality", label: "IoT Quality Check", icon: Shield, x: 50, y: 110, color: "text-purple-400" },
]
...
export function SmartContractBuilder({ onClose }: { onClose: () => void }) {
  ...
  const handleSign = () => {
      if (clauses.length < 3) {
          toast.error("Contrat incomplet", { description: "Veuillez sélectionner au moins 3 conditions." })
          return
      }
      setIsSigned(true)
    ...
    >
        ...
        <div className="flex items-center justify-between p-4 border-b border-slate-700 bg-slate-800/50 backdrop-blur">
             <div className="flex items-center gap-3">
                 ...
                 <div>
                     <h3 className="text-sm font-bold text-white uppercase tracking-wider">Builder de Contrat</h3>
        ...
        <div className="flex-1 grid grid-cols-12 overflow-hidden">
            ...
            <div className="col-span-4 border-r border-slate-700/50 p-4 space-y-4 bg-slate-900/30 overflow-y-auto custom-scrollbar">
                 <span className="text-[10px] uppercase font-bold text-slate-500 block mb-2">Conditions Disponibles</span>
                 ...
                 <div className="space-y-2">
                     {[
                         ...
                         >
                             <div className={`p-1.5 rounded ${clauses.includes(item.id) ? 'bg-blue-600 text-white' : 'bg-slate-700 text-slate-300'}`}>
                                 <item.icon size={12} />
                             </div>
            ...
            <div className="col-span-8 bg-[url('/grid-pattern.svg')] relative flex flex-col items-center justify-center p-8">
                ...
                {isSigned ? (
                    ...
                    >
                         ...
                         <div>
                             <h2 className="text-xl font-black text-white uppercase tracking-widest">Contrat Signé</h2>
                             <p className="text-xs text-emerald-400 font-mono mt-1">Transaction enregistrée sur la Blockchain</p>
                         </div>
                    </motion.div>
                ) : (
                    <>
                                ...
                                {clauses.map((c, i) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <div className="absolute top-1/2 left-1/2 w-[80px] h-[1px] bg-blue-500/30 origin-left -z-10" 
                        ...
                        <div className="absolute bottom-8 w-full px-12">
                             <Button onClick={handleSign} className="w-full h-12 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white font-bold uppercase tracking-widest shadow-lg flex items-center justify-center gap-3 group">
