...
const CommandMap: React.FC<CommandMapProps> = ({ 
  ...
  const loadsGeoJson = useMemo(() => ({
    ...
    features: loads.map(load => ({
      ...
      geometry: {
        type: 'Point',
        coordinates: [load.origin.lon, load.origin.lat]
      },
      properties: {
        id: load.id,
  ...
  const trucksGeoJson = useMemo(() => ({
    ...
    features: trucks.map(truck => ({
      ...
      geometry: {
        type: 'Point',
        coordinates: [truck.currentPosition.lon, truck.currentPosition.lat]
      },
      properties: {
        id: truck.id,
        weight: 1
  ...
  const loadHeatmapLayer = {
    ...
    paint: {
      ...
      'heatmap-color': [
        ...
        ['heatmap-density'],
        0, 'rgba(255,0,0,0)',
        0.2, 'rgba(255,100,0,0.5)',
        0.4, 'rgba(255,50,0,0.8)',
  ...
  const truckHeatmapLayer = {
    ...
    paint: {
      ...
      'heatmap-color': [
        ...
        0.4, 'rgba(0,50,255,0.8)',
        0.6, 'rgba(0,0,255,0.9)',
        1, 'rgba(0,0,255,1)'
      ],
      'heatmap-radius': ['interpolate', ['linear'], ['zoom'], 0, 2, 9, 20],
      'heatmap-opacity': 0.6
    ...
    <div className="map-wrapper" style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      >
        ...
        {loads.map(load => (
          ...
          >
            <div 
              className={`map-marker load-marker pulse-agri ${selectedLoad?.id === load.id ? 'selected scale-125' : ''}`}
              onMouseEnter={() => setHoverInfo({
                longitude: load.origin.lon,
                latitude: load.origin.lat,
                title: load.productType,
                details: `${load.quantity}t → ${load.destination.city} | ${load.priceOffer.toLocaleString()} F`,
                type: 'load'
              })}
        ...
        {trucks.map(truck => (
          <Marker
            key={truck.id}
            longitude={truck.currentPosition.lon}
            latitude={truck.currentPosition.lat}
            anchor="bottom"
            onClick={e => {
              e.originalEvent.stopPropagation();
          ...
          >
            <div 
              className={`map-marker truck-marker pulse-logistics ${selectedTruck?.id === truck.id ? 'selected scale-125' : ''}`}
              onMouseEnter={() => setHoverInfo({
                longitude: truck.currentPosition.lon,
                latitude: truck.currentPosition.lat,
        ...
        {hoverInfo && (
          ...
          >
            <div className={`p-1 font-mono text-[10px] ${hoverInfo.type === 'load' ? 'text-agri-neon' : 'text-logistics-neon'}`}>
              <div className="font-black uppercase border-b border-white/10 mb-1 pb-1">{hoverInfo.title}</div>
            ...
            <Source 
              ...
              data={{
                ...
                geometry: {
                  type: 'LineString',
                  coordinates: [
                    [selectedTruck.currentPosition.lon, selectedTruck.currentPosition.lat],
                    [selectedLoad.origin.lon, selectedLoad.origin.lat]
                  ]
                },
                properties: {}
              }}
              ...
              <Source 
                ...
                data={{
                  ...
                  geometry: {
                    ...
                    coordinates: [
                      [selectedTruck.currentPosition.lon, selectedTruck.currentPosition.lat],
                      [(selectedTruck.currentPosition.lon + selectedLoad.origin.lon) / 2 + 0.1, (selectedTruck.currentPosition.lat + selectedLoad.origin.lat) / 2 + 0.1],
                      [selectedLoad.origin.lon, selectedLoad.origin.lat]
                    ]
                  },
                  properties: {}
