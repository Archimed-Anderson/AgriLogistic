...
import { CheckCircle, TrendingUp, TrendingDown, Droplets, Sun, Cloud, CloudRain } from 'lucide-react'
  ...
  return (
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        {/* Health Badge */}
        <div className={`absolute top-4 right-4 px-3 py-1.5 rounded-xl ${healthStatus.color} backdrop-blur-sm border-2 border-white/20`}>
      ...
      <div className="p-6">
        ...
        <div className="mb-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-bold text-slate-600 uppercase tracking-wide">Santé Globale</span>
            <span className="text-lg font-black text-[#0A2619]">{zone.healthScore}%</span>
          </div>
          <div className="h-3 bg-slate-100 rounded-full overflow-hidden">
            <div 
              className={`h-full rounded-full transition-all duration-1000 ${
                zone.healthScore >= 85 ? 'bg-gradient-to-r from-emerald-500 to-teal-600' :
                zone.healthScore >= 70 ? 'bg-gradient-to-r from-green-500 to-emerald-600' :
                zone.healthScore >= 50 ? 'bg-gradient-to-r from-orange-500 to-amber-600' :
        ...
        <div className="flex items-center justify-between pt-4 border-t border-slate-100">
          ...
          <div className={`px-3 py-1 rounded-lg border text-xs font-bold ${getIrrigationColor(zone.irrigationStatus)}`}>
