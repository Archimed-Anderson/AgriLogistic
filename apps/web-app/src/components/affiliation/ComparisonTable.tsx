        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-[#141414]">
              <th className="p-6 text-left text-slate-400 font-black uppercase text-xs tracking-widest border-r border-white/5 w-1/4">
                Critères
              </th>
              {products.map((p, i) => (
                <th
                ...
                >
                  <div className="flex flex-col items-center group">
                    ...
                    <div className="text-yellow-500 text-[10px] font-black uppercase mb-1">
                      {p.category}
                    </div>
                    <div className="text-white font-black text-center text-sm line-clamp-1 uppercase px-4">
                      {p.name}
                    </div>
                  </div>
                </th>
          ...
          <tbody className="divide-y divide-white/5">
            ...
            <tr className="hover:bg-white/[0.02] transition-colors">
              <td className="p-6 font-black text-slate-300 uppercase text-xs border-r border-white/5">
                Score Global
              </td>
              {products.map((p, i) => (
                <td
            ...
            {/* Pros */}
            <tr className="hover:bg-white/[0.02] transition-colors">
              <td className="p-6 font-black text-emerald-500 uppercase text-xs border-r border-white/5">
                Points Forts
              </td>
              {products.map((p, i) => (
                <td
                  key={p.id}
                  className={cn('p-6 border-white/5', i !== products.length - 1 && 'border-r')}
                >
                  <ul className="space-y-2">
                    {p.pros.slice(0, 3).map((pro, j) => (
                      <li
                        key={j}
                        className="flex items-start gap-2 text-[11px] text-slate-300 font-bold leading-tight"
                      >
            ...
            {/* Cons */}
            <tr className="hover:bg-white/[0.02] transition-colors">
              <td className="p-6 font-black text-red-500 uppercase text-xs border-r border-white/5">
                Points Faibles
              </td>
              {products.map((p, i) => (
                <td
