...
export function HarvestInputModule() {
  const { parcels, addHarvest, addPendingHarvest, pendingHarvests, clearPendingHarvests } = useFarmerStore()
      ...
      <div className="relative h-[300px] w-full rounded-[3rem] overflow-hidden shadow-2xl group">
        ...
        <div className="absolute bottom-12 left-12 space-y-4">
          ...
          >
            ...
            <span className="text-[10px] font-black text-white uppercase tracking-[0.3em]">Module de Saisie Stratégique</span>
          </motion.div>
          <motion.h1 
            initial={{ opacity: 0, x: -30 }} 
            animate={{ opacity: 1, x: 0 }}
          ...
          >
            Optimisez chaque grain avec l'Intelligence Artificielle d'AgroDeep. 
            Capture GPS, diagnostic spectral et prévisions de rendement en temps réel.
          </motion.p>
        </div>
            ...
            {!isOffline && pendingHarvests.length > 0 && (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="text-white font-black text-sm uppercase tracking-tight">Synchronisation Requise</h4>
                    <p className="text-emerald-500/60 font-bold text-xs">{pendingHarvests.length} enregistrements locaux en attente.</p>
                  </div>
                </div>
                <Button 
                  onClick={syncAll} 
          ...
          <Card className="border-none bg-white dark:bg-[#0a1f18] shadow-[0_50px_100px_-20px_rgba(0,0,0,0.3)] rounded-[3rem] overflow-hidden">
            <CardHeader className="p-10 border-b border-slate-50 dark:border-white/5">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <CardTitle className="text-2xl font-black text-[#1B4D3E] dark:text-white tracking-widest uppercase">FICHE DE RÉCOLTE</CardTitle>
                  <CardDescription className="text-slate-400 font-bold text-xs uppercase tracking-widest">Enregistrement certifié AgroDeep Core</CardDescription>
                </div>
              </div>
            </CardHeader>
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-10">
                <div className="grid gap-10 md:grid-cols-2">
                  <div className="space-y-4">
                    <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em] ml-2">SÉLECTION PARCELLE</Label>
                  ...
                  <div className="space-y-4">
                    <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em] ml-2">TYPE DE CULTURE</Label>
                    <Input 
                  ...
                  <div className="space-y-4">
                    <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em] ml-2">QUANTITÉ NETTE</Label>
                    <div className="relative">
                      <Input 
                        type="number" 
                        step="0.1"
                      ...
                      />
                      <Badge className="absolute right-4 top-1/2 -translate-y-1/2 bg-[#1B4D3E] text-white border-none font-black text-[10px]">TONNES</Badge>
                    </div>
                  </div>

                  <div className="space-y-4">
                    <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em] ml-2">INDICE QUALITÉ</Label>
                ...
                <div className="bg-slate-50 dark:bg-white/5 rounded-3xl p-8 space-y-6 border border-slate-100 dark:border-white/5">
                  <div className="flex items-center justify-between">
                    <div className="space-y-1">
                      <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em]">POSITION GPS</Label>
                      <p className="text-xs font-bold text-[#1B4D3E] dark:text-[#D4A017]">Capture du point d'origine requis pour traçabilité Blockchain</p>
                    </div>
                    <Button 
                      type="button" 
                      onClick={captureGPS} 
                    ...
                    >
                      {isLocating ? <Loader2 className="h-4 w-4 animate-spin" /> : <MapPin className="mr-2 h-4 w-4" />}
                      CAPTURE LIVE
                    </Button>
                  </div>
                ...
                <div className="space-y-4">
                  <Label className="uppercase text-[11px] font-black text-slate-400 tracking-[0.2em] ml-2">NOTES DE TERRAIN</Label>
                  <Textarea 
            ...
            <Card className="border-none bg-[#050505] text-white shadow-3xl rounded-[3rem] overflow-hidden relative group">
              ...
              <CardHeader className="p-10 relative border-b border-white/5">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <CardTitle className="text-xl font-black tracking-widest">DIAGNOSTIC VISUEL IA</CardTitle>
                    <Badge variant="outline" className="border-white/10 text-white/40 uppercase text-[8px] font-black">Powered by AgroLink Vision</Badge>
                  </div>
                </div>
              </CardHeader>
                ...
                >
                  {!isDiagnosticRunning && !diagnosticResult ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center space-y-4">
                      ...
                      <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] text-center px-12">Capture d'image pour analyse de contamination</p>
                    </div>
                  ) : isDiagnosticRunning ? (
                    <div className="absolute inset-0 flex flex-col items-center justify-center space-y-6">
                      <Loader2 className="h-12 w-12 text-[#D4A017] animate-spin" />
                      <div className="text-center">
                        <p className="text-sm font-black text-white tracking-widest uppercase mb-1">Analyse en cours</p>
                        <p className="text-[10px] font-bold text-white/40 uppercase tracking-widest">Séquençage spectral v4.2</p>
                      </div>
                      <motion.div 
                    ...
                    <div className="absolute inset-0 flex items-center justify-center bg-emerald-500/10 p-10 text-center">
                      <div className="space-y-4">
                        ...
                        <p className="font-black text-lg tracking-tight uppercase leading-snug">{diagnosticResult}</p>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <Card className="border-none bg-white dark:bg-[#0a1f18] shadow-3xl rounded-[3rem] overflow-hidden">
               <CardHeader className="p-10 border-b border-slate-50 dark:border-white/5">
                 <div className="flex items-center gap-4">
                   ...
                   <div>
                    <CardTitle className="text-xl font-black tracking-widest uppercase">SYSTÈME PRÉDICTIF</CardTitle>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Algorithme YieldPredict Premium</p>
                   </div>
                 </div>
               </CardHeader>
               <CardContent className="p-10 space-y-8">
                  <div className="flex flex-col items-center justify-center py-6 bg-slate-50 dark:bg-black/20 rounded-[2rem] border border-slate-100 dark:border-white/5">
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em] mb-4">Estimation du Rendement Final</p>
                    <div className="text-7xl font-black text-[#1B4D3E] dark:text-white tracking-tighter">
                      {isEstimating ? (
                        <div className="flex gap-2">
                          ...
                          <motion.span animate={{ opacity: [1, 0.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.2 }}>•</motion.span>
                          <motion.span animate={{ opacity: [1, 0.2, 1] }} transition={{ repeat: Infinity, duration: 1, delay: 0.4 }}>•</motion.span>
                        </div>
                      ) : aiEstimate ? (
                        `${aiEstimate}` 
                      ) : (
                      ...
                      {aiEstimate && <span className="text-xl text-slate-400 dark:text-slate-600 ml-2 italic">T/ha</span>}
                    </div>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3 p-4 bg-slate-50 dark:bg-white/2 rounded-2xl">
                      ...
                      <p className="text-[9px] font-bold text-slate-400 uppercase leading-loose">
                        Calcul basé sur 14 points de données dont l'indice NDVI et l'humidité résiduelle du sol.
                      </p>
                    </div>
                  </div>
               </CardContent>
