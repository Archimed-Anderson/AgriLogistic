...
import { Sprout, CloudRain, Wind, Droplets, AlertTriangle, CheckCircle2, Play, Users, BarChart3, ArrowRight } from "lucide-react"
...
const Farm3DViewer = dynamic(() => import('@/components/products/farm/Farm3DViewer').then(mod => mod.Farm3DViewer), { 
  ssr: false,
  loading: () => <div className="w-full h-full min-h-[400px] bg-slate-900 rounded-3xl animate-pulse border border-slate-700 flex items-center justify-center text-green-500 font-mono text-sm">LOADING DIGITAL TWIN...</div>
})
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 font-sans selection:bg-green-500/30">
      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="container relative z-10 mx-auto px-4 text-center">
          <Badge variant="outline" className="mb-6 border-green-500 text-green-400 uppercase tracking-widest backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Button size="lg" className="h-14 px-8 text-lg font-bold bg-green-600 hover:bg-green-500 shadow-xl shadow-green-900/20 rounded-full">
              Essayer la Démo
            </Button>
            <Button size="lg" variant="outline" className="h-14 px-8 text-lg font-bold border-white/20 text-white hover:bg-white/10 rounded-full backdrop-blur-sm">
        ...
        <div className="bg-white dark:bg-slate-800 rounded-3xl shadow-2xl border border-slate-200 dark:border-slate-700 p-6 md:p-8 backdrop-blur-xl">
          <div className="flex items-center justify-between mb-8">
            ...
            <div className="hidden md:flex items-center gap-4 bg-blue-50 dark:bg-blue-900/20 px-4 py-3 rounded-xl border border-blue-100 dark:border-blue-800">
              ...
              <div>
                <div className="text-xs font-bold text-blue-700 dark:text-blue-300">Météo en Direct (Yamoussoukro)</div>
                <div className="text-lg font-black">{farmWeather.current.temp}°C • {farmWeather.current.condition}</div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[600px] lg:h-[500px]">
            ...
            <div className="flex flex-col gap-4 overflow-y-auto pr-2">
              <h3 className="font-bold text-sm text-muted-foreground uppercase tracking-widest mb-2">Vos Parcelles</h3>
              {farmPlots.map((plot) => (
                <div 
                  key={plot.id} 
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    ...
                    <Badge className={
                       ...
                       plot.status === 'warning' ? 'bg-amber-500' : 'bg-red-500'
                    }>{plot.healthScore}% S.</Badge>
                  </div>
                  <div className="grid grid-cols-2 text-xs text-muted-foreground gap-y-1">
                    <span>Culture : <span className="font-medium text-foreground">{plot.crop}</span></span>
                    <span>Stade : <span className="font-medium text-foreground">{plot.stage}</span></span>
      ...
      <section className="py-24 bg-slate-50 dark:bg-slate-900">
        <div className="container mx-auto px-4">
          <div className="flex flex-col md:flex-row items-end justify-between mb-12 gap-6">
            <div>
              <h2 className="text-3xl font-black tracking-tighter mb-2 bg-gradient-to-r from-green-600 to-emerald-500 bg-clip-text text-transparent">Académie AgriLogistic</h2>
              <p className="text-lg text-muted-foreground font-medium max-w-2xl">
                Accédez à notre bibliothèque de contenus : guides techniques, analyses de marché et tutoriels vidéo.
              </p>
            </div>
            {/* Quick Filters */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                {(activeTab === "Tous" || activeTab === "Tutoriel") && (
                    <div className="space-y-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                        ...
                        <div>
                            ...
                            <p className="text-muted-foreground">Une introduction essentielle aux enjeux de la digitalisation pour les exploitations agricoles modernes en Afrique.</p>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-6">
                     <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-700 pb-4">
                        ...
                        <span className="text-xs font-medium text-muted-foreground bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-md">
                            {farmArticles.filter(a => activeTab === "Tous" || a.category === activeTab).length} résultats
                        </span>
                     </div>
                        ...
                        >
                            ...
                            <div className="flex flex-col flex-1 py-1">
                                ...
                                <div className="flex items-center justify-between border-t border-slate-50 dark:border-slate-700 pt-4 mt-4">
                                    <div className="flex items-center gap-2">
                                        <span className="text-xs font-bold text-slate-400 uppercase tracking-wide">{article.author}</span>
                                    </div>
                    ...
                    {/* Empty State */}
                    {farmArticles.filter(a => activeTab === "Tous" || a.category === activeTab).length === 0 && (
            ...
            <div className={`lg:col-span-4 space-y-8 ${activeTab !== "Tous" ? "opacity-50 pointer-events-none grayscale transition-all" : "transition-all"}`}>
                ...
                <div className="bg-green-900 rounded-3xl p-8 text-white relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-green-100 mb-6 text-sm">Recevez chaque semaine nos conseils agronomiques et les tendances du marché directement par email.</p>
                ...
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {["#AgriTech", "#CacaoDurable", "#Irrigation", "#Drones", "#FinTech", "#Climat", "#BioControls"].map(tag => (
                            <span key={tag} className="px-3 py-1 bg-slate-100 dark:bg-slate-900 rounded-lg text-xs font-medium text-slate-600 dark:text-slate-400 hover:text-green-600 hover:bg-green-50 cursor-pointer transition-colors block">
                                {tag}
                            </span>
                        ))}
                    </div>
                ...
                {/* Download Resource */}
                <a href="#" className="block bg-blue-50 dark:bg-blue-900/10 rounded-3xl p-6 border border-blue-100 dark:border-blue-800 hover:shadow-md transition-shadow group">
                    ...
                    <h4 className="font-bold text-lg mb-2 group-hover:text-blue-600 transition-colors">Calendrier Cultural 2026</h4>
                    <p className="text-sm text-slate-500 mb-4">Téléchargez le guide complet des semis et récoltes pour la zone Afrique de l'Ouest.</p>
      ...
      <section className="py-24 bg-green-900 relative overflow-hidden">
         ...
         <div className="container mx-auto px-4 relative z-10 text-center">
            <h2 className="text-3xl md:text-5xl font-black text-white mb-8 tracking-tight">Prêt à digitaliser votre exploitation ?</h2>
            <p className="text-green-100 text-xl max-w-2xl mx-auto mb-10">Rejoignez plus de 5,000 agriculteurs qui utilisent AgriLogistic Farm pour sécuriser leurs récoltes.</p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
               <Button size="lg" variant="secondary" className="h-14 px-10 text-lg font-bold">
                 Créer un compte
               </Button>
               <Button size="lg" variant="outline" className="h-14 px-10 text-lg font-bold text-white border-white/20 hover:bg-white/10">
                 Contacter un expert
               </Button>
            </div>
         </div>
