...
if ($port5432) {
    ...
    if ($choice -eq "1") {
        ...
        docker network create AgriLogistic-network 2>$null
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            if (Test-Path .env) {
                ...
                $content = $content -replace "DB_PASSWORD=.*", "DB_PASSWORD=AgriLogistic_password"
            ...
            docker ps --filter "name=AgriLogistic" --format "table {{.Names}}\t{{.Status}}"
        ...
        Write-Host "  CREATE DATABASE AgriLogistic_auth;" -ForegroundColor White
        Write-Host "  CREATE USER AgriLogistic WITH PASSWORD 'AgriLogistic_password';" -ForegroundColor White
        Write-Host "  GRANT ALL PRIVILEGES ON DATABASE AgriLogistic_auth TO AgriLogistic;" -ForegroundColor White
        Write-Host "  ALTER USER AgriLogistic CREATEDB;" -ForegroundColor White
