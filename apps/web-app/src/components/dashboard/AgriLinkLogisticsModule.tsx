...
const DynamicLiveMap = dynamic(() => import('./LiveLogisticMap'), {
  ssr: false,
  loading: () => (
    <div className="h-full w-full bg-[#050505] flex items-center justify-center text-[#D4A017] animate-pulse">
...
const DRIVERS = [
  {
    ...
    hours: '142h',
    avatar:
      'https://images.unsplash.com/photo-1544723795-3fb6469f5b39?auto=format&fit=crop&q=80&w=100',
    status: 'Driving',
  },
  {
    id: 2,
  ...
  },
  {
    ...
    hours: '128h',
    avatar:
      'https://images.unsplash.com/photo-1542909168-82c3e7fdca5c?auto=format&fit=crop&q=80&w=100',
    status: 'Loading',
  },
];
  ...
  return (
    <Card className="bg-[#0a0a0a] border-white/5 p-6 space-y-4 shadow-2xl relative overflow-hidden group">
      ...
      <div className="flex justify-between items-start relative z-10">
        ...
        <Badge className="bg-white/5 text-slate-400 border-none text-[10px] font-black">
          {stat.change}
        </Badge>
      </div>
    ...
    <Card className="bg-[#0a0a0a] border-white/5 rounded-[3rem] p-10 space-y-8 shadow-3xl overflow-hidden relative">
      ...
      <div className="space-y-2 relative z-10">
        <h3 className="text-2xl font-black text-white uppercase tracking-tighter italic">
          PLANIFICATEUR <span className="text-[#D4A017]">IA</span>
        </h3>
      ...
      <div className="space-y-4 relative z-10">
        <div className="flex gap-3">
          ...
          <Button
            onClick={addPoint}
            className="bg-[#D4A017] hover:bg-amber-600 text-black h-14 w-14 rounded-2xl shrink-0"
          >
            <MapPin size={22} />
          </Button>
        </div>
        ...
        <div className="space-y-3">
          {points.map((p, i) => (
            <motion.div
              ...
              key={i}
              className="flex items-center gap-4 p-4 bg-white/2 rounded-2xl border border-white/5"
            >
              <div className="h-6 w-6 rounded-full bg-blue-500/20 text-blue-400 flex items-center justify-center text-[10px] font-black">
                {i + 1}
              </div>
              <span className="text-[10px] font-black text-white uppercase">{p}</span>
              <button className="ml-auto text-slate-600 hover:text-red-400">
                <Trash2 size={14} />
              </button>
            </motion.div>
          ))}
      ...
      <Button
        ...
        className="w-full bg-[#1B4D3E] hover:bg-emerald-700 text-white rounded-2xl h-16 font-black gap-3 shadow-2xl relative z-10 uppercase tracking-widest text-xs"
      >
...
function DeliveryValidation() {
  ...
  const handleSign = () => {
    ...
    toast.success('SIGNATURE ENREGISTRÉE', {
      description: 'Certificat de livraison prêt pour validation blockchain.',
    });
  };
  ...
  return (
    <Card className="bg-[#0a0a0a] border-[#D4A017]/20 border rounded-[3rem] p-10 space-y-8 shadow-[0_30px_60px_rgba(212,160,23,0.1)]">
      <div className="space-y-2">
        <Badge className="bg-[#D4A017] text-black font-black text-[10px] px-4 py-1 rounded-full mb-2">
          VALIDATION_FINALE
        </Badge>
        ...
        <button
          ...
          className={cn(
            ...
            signature
              ...
              : 'bg-white/2 border-white/10 hover:border-[#D4A017]/40'
          )}
        >
          {signature ? (
            <CheckCircle2 className="text-emerald-400 h-10 w-10" />
          ) : (
            <PenTool className="text-[#D4A017] h-10 w-10" />
          )}
          <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
            {signature ? 'Signé' : 'Signer'}
          </span>
        </button>
      </div>
      ...
      <Button
        disabled={!signature}
        className="w-full bg-[#D4A017] hover:bg-amber-600 text-black rounded-2xl h-16 font-black gap-3 shadow-2xl transition-all uppercase tracking-widest text-xs"
      >
    ...
    <div className="max-w-[1800px] mx-auto space-y-10 p-2">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 font-bold uppercase text-[11px] tracking-[0.5em] mt-2 flex items-center gap-3">
                <span className="h-2 w-2 rounded-full bg-emerald-500 animate-pulse" /> Surveillance
                Fleet-IQ v4.2 | 12 Unités en Ligne
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4">
          {['SÉCURITÉ: OK', 'GPS: OPTIMAL', 'SAT: 12/12'].map((txt, i) => (
            <Badge
              key={i}
              variant="outline"
              className="border-white/10 text-white/40 font-black text-[9px] px-4 py-2 rounded-xl bg-white/5 tracking-widest uppercase"
            >
              {txt}
            </Badge>
          ))}
      ...
      <div className="grid lg:grid-cols-12 gap-10">
            ...
            <div className="absolute top-10 left-10 z-20 flex flex-col gap-4">
              <div className="bg-black/80 backdrop-blur-xl p-5 rounded-3xl border border-white/10 space-y-1">
                ...
                <p className="text-2xl font-black text-white italic">
                  14h 22m{' '}
                  <span className="text-xs text-emerald-500 font-bold not-italic ml-2">-15%</span>
                </p>
              </div>
        ...
        <div className="lg:col-span-4 space-y-10">
          <Card className="bg-[#0a0a0a] border-white/10 rounded-[3.5rem] p-10 space-y-8 shadow-3xl min-h-[500px]">
            <div className="flex items-center justify-between">
              <h3 className="text-2xl font-black text-white tracking-tighter uppercase italic">
                MONITEUR <span className="text-[#D4A017]">FLOTTE</span>
              </h3>
              <Badge className="bg-[#1B4D3E] text-[#D4A017] px-4 py-1.5 rounded-full font-black text-[10px]">
                SCAN EN COURS...
              </Badge>
            </div>
            ...
            <div className="space-y-6">
              {activeTrucks.map((truck) => (
                <motion.div
                  ...
                  key={truck.id}
                  className="p-6 bg-white/5 rounded-[2.5rem] border border-white/5 hover:border-[#D4A017]/30 transition-all cursor-pointer group"
                >
                  <div className="flex gap-5 mb-5">
                    <div className="h-16 w-16 rounded-2xl bg-black overflow-hidden border border-white/10 group-hover:border-[#D4A017]/40 transition-colors">
                      <img
                        src="https://images.unsplash.com/photo-1586191582151-f737730a1632?auto=format&fit=crop&q=80&w=200"
                  ...
                  <div className="space-y-2">
                    ...
                    <Progress
                      value={truck.health}
                      className="h-1.5 bg-black [&>div]:bg-[#D4A017]"
                    />
                  </div>
                </motion.div>
              ))}
          ...
          <div className="space-y-6">
            <h4 className="text-xl font-black text-white uppercase italic ml-6">
              ÉQUIPAGE <span className="text-[#D4A017]">CONNECTÉ</span>
            </h4>
            <div className="grid grid-cols-1 gap-4">
              {DRIVERS.map((driver) => (
                <div
                  key={driver.id}
                  className="bg-[#0a0a0a] border border-white/5 p-6 rounded-[2.5rem] flex items-center justify-between group hover:border-emerald-500/30 transition-all"
                >
                  <div className="flex items-center gap-5">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 text-[9px] font-black text-slate-500 uppercase">
                        <Star size={10} className="text-[#1B4D3E] fill-[#1B4D3E]" /> {driver.rating}{' '}
                        | {driver.hours} de route
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-none font-black text-[8px] px-3">
                      {driver.status}
                    </Badge>
                  </div>
                </div>
          ...
          <Card className="bg-gradient-to-br from-[#1B4D3E] to-[#0a1f18] border-none rounded-[3.5rem] p-10 relative overflow-hidden group">
            ...
            <div className="relative z-10 space-y-6">
              <Badge className="bg-white/10 text-white font-black text-[9px] px-4 py-1.5 rounded-full uppercase tracking-widest border border-white/20">
                Mobile HUB v4.0
              </Badge>
              <h4 className="text-3xl font-black text-white tracking-tighter uppercase leading-[0.9]">
                PILOTAGE <br /> CHAUFFEUR
              </h4>
              <p className="text-white/60 text-xs font-bold uppercase leading-relaxed">
                Installez l'Agent AgriLink sur les terminaux de votre flotte pour synchroniser la
                blockchain.
              </p>
              <div className="flex gap-4">
                <Button
                  variant="outline"
                  className="flex-1 border-white/20 text-white rounded-xl h-14 font-black text-[10px] uppercase"
                >
                  APP STORE
                </Button>
                <Button
