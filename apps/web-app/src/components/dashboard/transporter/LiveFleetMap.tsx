...
// --- Custom Icon Factory ---
const createTruckIcon = (heading: number, color: string = '#10b981') =>
  L.divIcon({
    ...
    const interval = setInterval(() => {
      setFleet((prev) =>
        prev.map((t) => ({
          ...
          speed:
            t.speed > 0
              ? Math.max(0, Math.min(100, t.speed + Math.floor(Math.random() * 5 - 2)))
              : 0,
        }))
      );
    }, 2000);
        ...
        <input
          ...
          onChange={(e) => {
            ...
            const newFleet = MOCK_FLEET.map((t) => ({
              ...t,
              lat:
                t.lat + ((t.speed * minutes) / 60) * 0.01 * Math.cos((t.heading * Math.PI) / 180),
              lng:
                t.lng + ((t.speed * minutes) / 60) * 0.01 * Math.sin((t.heading * Math.PI) / 180),
