...
export function FarmerOverview() {
  ...
  const [prediction, setPrediction] = React.useState<{ yield: number; confidence: number } | null>(null)
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-6 p-6 bg-white dark:bg-[#050505] rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-2xl overflow-hidden relative group">
         ...
         <div className="flex items-center gap-6 relative z-10">
            <div className="flex -space-x-2">
               {[1, 2, 3, 4].map(i => (
                 <div key={i} className="h-10 w-10 rounded-full border-4 border-white dark:border-[#050505] bg-slate-100 dark:bg-white/10 flex items-center justify-center overflow-hidden">
            ...
            <div className="space-y-1">
               <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Opérateurs Connectés</p>
               <p className="text-sm font-black text-[#1B4D3E] dark:text-emerald-500 uppercase">Synchronisation Satellite Active</p>
            </div>
         </div>
         ...
         <div className="flex items-center gap-10 relative z-10">
            ...
            <div className="flex gap-4">
               <Badge className="bg-emerald-500/10 text-emerald-600 border-none px-4 py-2 rounded-full font-black text-[10px] uppercase">AgriLink OK</Badge>
               <Badge className="bg-blue-500/10 text-blue-600 border-none px-4 py-2 rounded-full font-black text-[10px] uppercase">Blockchain SYNC</Badge>
            </div>
         </div>
      </div>
                ...
                <div className="space-y-4">
                   <div className="flex justify-between items-center">
                      <div className="p-4 bg-white/10 rounded-2xl backdrop-blur-md">
                         {isPredicting ? <Loader2 className="h-6 w-6 animate-spin" /> : <Cpu className="h-6 w-6" />}
                      </div>
                      <Badge className="bg-white/10 text-white border-none font-black text-[8px] tracking-[0.2em] uppercase">Moteur Neural v4</Badge>
                   </div>
                   <div className="space-y-1">
                      <h4 className="text-[10px] font-black text-white/50 uppercase tracking-widest">Rendement Prédit</h4>
                      <p className="text-4xl font-black tracking-tighter">
                         {prediction ? `${prediction.yield}` : "---"} <span className="text-lg text-white/40">T/HA</span>
                      </p>
                   </div>
                </div>
          ...
          <CardHeader className="p-10 border-b border-slate-50 dark:border-white/5 bg-slate-50/50 dark:bg-white/2">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                ...
                <CardDescription className="font-black text-[10px] uppercase tracking-widest text-slate-400">Analyse Spectral-Météo // 7 Jours</CardDescription>
              </div>
              <div className="flex gap-3">
                 {[
                   ...
                   <div key={i} className="flex items-center gap-2 px-4 py-2 bg-white dark:bg-black/20 rounded-2xl border border-slate-100 dark:border-white/10 shadow-sm">
          ...
          <CardContent className="p-10 h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={forecastData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                    ...
                    if (active && payload && payload.length) {
                       return (
                         <div className="p-4 bg-black/90 backdrop-blur-xl border border-white/10 rounded-2xl shadow-3xl space-y-2">
                           <p className="text-[10px] font-black text-white/40 uppercase tracking-widest">{payload[0].payload.day}</p>
                           <div className="flex gap-4">
                              <div className="space-y-1">
                                 <p className="text-[8px] font-black text-[#D4A017] uppercase">Température</p>
                                 <p className="text-xl font-black text-white">{payload[0].value}°C</p>
                              </div>
                              <div className="space-y-1 text-right">
                                 <p className="text-[8px] font-black text-emerald-500 uppercase">Humidité</p>
                ...
                />
                <Area 
                  type="monotone" dataKey="temp" stroke="#D4A017" strokeWidth={4} fill="url(#colorTemp)" 
                  dot={{ r: 4, fill: "#D4A017", strokeWidth: 0 }}
                />
                <Area 
                  type="monotone" dataKey="humidity" stroke="#1B4D3E" strokeWidth={4} fill="url(#colorHum)" 
           ...
           <CardContent className="p-10 space-y-10 relative z-10">
                ...
                <div key={parcel.id} className="space-y-4">
                  <div className="flex justify-between items-end">
                    <div className="space-y-1">
                      ...
                      <p className="text-[10px] text-white/30 uppercase font-black tracking-[0.2em]">{parcel.cropType}</p>
                    </div>
                    <div className="text-right">
                       <p className="text-2xl font-black text-[#D4A017] tracking-tighter">{parcel.healthScore}%</p>
                       <p className="text-[8px] font-black text-emerald-500 uppercase">Optimal</p>
                    </div>
                  </div>
                  <div className="h-2 w-full bg-white/5 rounded-full overflow-hidden">
                     {/* @ts-ignore */}
                     <Progress value={parcel.healthScore} className="h-full bg-gradient-to-r from-emerald-900 via-emerald-500 to-[#D4A017]" />
                  </div>
                </div>
              ))}
              ...
              <div className="pt-10 border-t border-white/5">
                 <div className="flex items-center gap-6 p-6 bg-white/5 rounded-[2rem] border border-white/5 hover:border-[#D4A017]/30 transition-all cursor-pointer group">
                    ...
                    <div>
                       <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.3em]">IA STRATÉGIQUE</p>
                       <p className="text-xs font-bold text-amber-500 leading-tight uppercase">Irritation recommandée : Zone Sud-02 (Urgence faible)</p>
                    </div>
                 </div>
              </div>
           </CardContent>
...
function KPIItem({ title, value, unit, description, icon, trend, percentage, color }: any) {
  ...
  const trendIcon = trend === 'up' ? <ArrowUpRight className="h-3 w-3" /> : trend === 'down' ? <ArrowDownRight className="h-3 w-3" /> : <Activity className="h-3 w-3" />
      ...
      <Card className="border-none bg-white dark:bg-[#0a1f18] shadow-3xl rounded-[3rem] overflow-hidden group hover:border-[#D4A017]/50 border border-slate-100 dark:border-white/5 transition-all duration-300 h-full">
        <CardContent className="p-10 space-y-8 flex flex-col justify-between h-full">
          <div className="flex justify-between items-start">
            <div className={cn("p-4 rounded-2xl shadow-xl transition-transform group-hover:scale-110", accentColor)}>{icon}</div>
          ...
          <div className="space-y-2">
             <div className="flex items-baseline gap-2">
                <span className="text-5xl font-black text-[#1B4D3E] dark:text-white tracking-tighter leading-none">{value}</span>
                <span className="text-xl font-black text-slate-300 dark:text-slate-600 uppercase">{unit}</span>
             </div>
             <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wide leading-relaxed">{description}</p>
          </div>
        </CardContent>
      </Card>
    </motion.div>
