  ...
  return (
    <motion.div 
      ...
      className={`load-card-premium p-4 rounded-xl cursor-pointer transition-all border ${
        isSelected ? 'border-agri-neon bg-agri-neon/5 ring-1 ring-agri-neon/30' : 'border-white/5 hover:border-white/20'
      } ${load.isNew ? 'pulse-agri' : ''}`}
      onClick={onClick}
    >
      <div className="flex justify-between items-start mb-2">
        <span className="text-[10px] font-black tracking-tighter text-gray-400 uppercase">#{load.id.slice(-6)}</span>
        {load.aiMatchScore && (
      ...
      <h4 className="text-white font-black text-sm mb-1 uppercase tracking-tight">{load.productType}</h4>
      
      <div className="flex items-center gap-2 text-xs text-gray-400 mb-3">
        <span className="font-bold text-white/80">{load.origin.city}</span>
        <span className="text-agri-neon/40">→</span>
        <span className="font-bold text-white/80">{load.destination.city}</span>
      </div>
