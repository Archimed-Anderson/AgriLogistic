...
export function AgriLogisticLink() {
  const [activeTab, setActiveTab] = useState<'overview' | 'loads' | 'trucks' | 'matches'>('overview');
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-card border rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow">
          ...
          <div className="space-y-1">
            ...
            <p className="text-2xl font-bold">{(mockAnalytics.totalRevenue / 1000000).toFixed(1)}M FCFA</p>
          </div>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 bg-card border rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-4">
            {mockMatches.slice(0, 5).map((match) => (
              <div key={match.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                <div className="flex items-center gap-4">
                  <div className="p-2 bg-green-50 dark:bg-green-900/10 rounded-full">
                    <CheckCircle className="h-5 w-5 text-green-600" />
                  </div>
                  <div>
                    <div className="font-medium">Chargement #{match.loadId.slice(-4)} → {match.truckId}</div>
    ...
    <div className="bg-card border rounded-xl shadow-sm overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            {mockLoads.filter(l => l.status !== 'Delivered').slice(0, 10).map((load) => (
              <tr key={load.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-900/50 transition-colors">
                <td className="px-6 py-4 font-medium">{load.id}</td>
                <td className="px-6 py-4">
                  <div className="font-medium">{load.productType}</div>
                  <div className="text-xs text-muted-foreground">{load.packaging || 'Standard'}</div>
                </td>
                <td className="px-6 py-4">{load.quantity} {load.unit}</td>
                <td className="px-6 py-4">
                  ...
                  {load.aiMatchScore ? (
                    <div className="flex items-center gap-2">
                       <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                         <div className={`h-full rounded-full ${load.aiMatchScore > 80 ? 'bg-green-500' : 'bg-yellow-500'}`} style={{width: `${load.aiMatchScore}%`}}></div>
                       </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Plateforme de mise en relation logistique intelligente</p>
        </div>
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 p-1 rounded-lg">
          <button 
            onClick={() => setActiveTab('overview')}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'overview' ? 'bg-white dark:bg-gray-700 shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Vue d'ensemble
          </button>
          <button 
            onClick={() => setActiveTab('loads')}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'loads' ? 'bg-white dark:bg-gray-700 shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Chargements
          </button>
          <button 
            onClick={() => setActiveTab('trucks')}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${activeTab === 'trucks' ? 'bg-white dark:bg-gray-700 shadow-sm text-foreground' : 'text-muted-foreground hover:text-foreground'}`}
          >
            Camions
          </button>
        </div>
