...
export default function LoueurPage() {
  ...
  const categoryCounts = {
    ...
    'MACHINES_TRAITEMENT': rentalEquipmentData.filter(eq => eq.category === 'MACHINES_TRAITEMENT').length,
    'MAINTENANCE_NETTOYAGE': rentalEquipmentData.filter(eq => eq.category === 'MAINTENANCE_NETTOYAGE').length,
    'MATERIAUX_CONSTRUCTION': rentalEquipmentData.filter(eq => eq.category === 'MATERIAUX_CONSTRUCTION').length
  }
          ...
          <div className="lg:col-span-3">
            ...
            <div className="bg-slate-800 rounded-2xl p-6 border-2 border-slate-700 mb-8">
              ...
              <div className="mt-4 text-slate-400 font-bold">
                <span className="text-yellow-500 text-lg">{filteredEquipment.length}</span> résultat{filteredEquipment.length > 1 ? 's' : ''}
              </div>
            </div>
            ...
            {filteredEquipment.length > 0 ? (
              ...
              <div className="text-center py-20 bg-slate-800 rounded-2xl border-2 border-slate-700">
                ...
                <h3 className="text-2xl font-black text-slate-400 mb-3 uppercase">Aucun équipement trouvé</h3>
