...
test.describe('Crop Intelligence - Validation Complete', () => {
  ...
  test('4. ALERTES: Panneau alertes IA visible', async ({ page }) => {
    ...
    const noAlertsMessage = await page
      .getByText(/Aucune alerte active/i)
      .isVisible()
      .catch(() => false);
  ...
  test('6. SCAN: Bouton Scan fonctionne avec animation', async ({ page }) => {
    ...
    const firstZoneScore = await page
      .locator('[class*="text-lg"][class*="font-black"]')
      .first()
      .textContent();
    ...
    await expect(page.getByText(/Analyse satellite en cours|Scan en cours/i)).toBeVisible({
      timeout: 1000,
    });
  ...
  test('7. FILTRES: Filtrage des zones fonctionne', async ({ page }) => {
    ...
    if (await healthyFilter.isVisible()) {
      ...
      const healthyZones = await page
        .locator('[class*="rounded-3xl"][class*="cursor-pointer"]')
        .count();
...
test.describe('Crop Intelligence - Tests Avancés', () => {
  ...
  test('Historique alertes toggle fonctionne', async ({ page }) => {
    ...
    const closeAlertButton = page
      .locator('button[title*="résolu"]')
      .or(page.locator('svg').filter({ hasText: /X|×/i }).locator('..'))
      .first();
