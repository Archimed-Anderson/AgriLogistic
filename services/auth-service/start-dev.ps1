...
if ($LASTEXITCODE -ne 0) {
    ...
    docker exec agrodeep-postgres sh -c "sed -i 's/host    all             all             127.0.0.1\/32            trust/host    all             all             127.0.0.1\/32            scram-sha-256/' /var/lib/postgresql/data/pg_hba.conf" | Out-Null
    docker exec agrodeep-postgres psql -U agrodeep -c "SELECT pg_reload_conf();" | Out-Null
...
Write-Host "    - PostgreSQL: agrodeep-postgres:5432 (via reseau Docker)" -ForegroundColor Gray
Write-Host "    - Redis: agrodeep-redis:6379 (via reseau Docker)" -ForegroundColor Gray
...
Write-Host "  Connexion via reseau Docker (agrodeep-network)" -ForegroundColor Cyan
