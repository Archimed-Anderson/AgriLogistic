        ...
        <div className="flex items-center justify-between">
              ...
              <NavigationMenuList>
                ...
                <NavigationMenuItem>
                  <Link href="/blog" legacyBehavior passHref>
                    <NavigationMenuLink className={cn(navigationMenuTriggerStyle(), "bg-transparent font-bold text-muted-foreground hover:text-primary transition-colors text-[13px] tracking-wide uppercase")}>
                      Blog & Insights
                    </NavigationMenuLink>
                  </Link>
                </NavigationMenuItem>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:flex items-center gap-4">
              <Link href="/marketplace/cart" className="relative p-2 text-primary hover:bg-primary/5 rounded-full transition-colors">
              ...
              <Link href="/login" className="text-[13px] font-black uppercase tracking-widest text-primary hover:bg-primary/5 px-4 py-2 rounded-xl transition-colors">
                Connexion
              </Link>
              <Link href="/register" className="bg-primary text-white text-[13px] font-black uppercase tracking-widest px-6 py-2.5 rounded-xl shadow-xl shadow-primary/20 hover:scale-105 transition-all">
                Rejoindre
              </Link>
            </div>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px] p-0 border-l-primary/10">
                ...
                <div className="flex flex-col h-full overflow-y-auto pb-20">
                  <div className="px-2 py-6 space-y-1">
                    <p className="px-4 text-[10px] font-black tracking-[0.2em] text-muted-foreground/40 mb-3 uppercase">Produits</p>
                    {products.map((p) => (
                      <Link 
                        key={p.title} 
                        href={p.href} 
                  ...
                  <div className="px-2 py-6 space-y-1 border-t border-primary/5">
                    <p className="px-4 text-[10px] font-black tracking-[0.2em] text-muted-foreground/40 mb-3 uppercase">Solutions</p>
                    {solutions.map((s) => (
                      <Link 
                        key={s.title} 
                        href={s.href} 
                  ...
                  <div className="px-6 py-8 border-t border-primary/5 mt-auto bg-slate-50/30 space-y-4">
                    <Link href="/login" onClick={() => setIsOpen(false)}>
                      <Button variant="outline" className="w-full h-14 font-black rounded-2xl border-primary/10 text-primary hover:bg-primary/5">Se connecter</Button>
                    </Link>
                    <Link href="/register" onClick={() => setIsOpen(false)}>
                      <Button className="w-full h-14 font-black rounded-2xl shadow-xl shadow-primary/20 bg-primary hover:bg-primary/90">Rejoindre maintenant</Button>
                    </Link>
                  </div>
                </div>
              </SheetContent>
      ...
      <NavigationMenuLink asChild>
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            {Icon && <Icon className="h-4 w-4 text-primary group-hover:scale-110 group-hover:rotate-3 transition-transform" />}
