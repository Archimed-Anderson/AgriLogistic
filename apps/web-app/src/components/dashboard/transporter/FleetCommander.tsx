...
function TruckModel({ status, onPartClick }: { status: string, onPartClick: (part: string) => void }) {
  ...
  return (
    <group>
      ...
      <mesh 
        ...
        onClick={(e) => { e.stopPropagation(); onPartClick("Moteur") }}
      ...
      {/* Status Lights */}
      <pointLight position={[2.5, 3, 0]} color={status === "Critical" ? "#ef4444" : "#10b981"} intensity={2} distance={5} />
    </group>
  )
}
...
const VEHICLES = [
   ...
   { id: "FL-03", model: "Renault T-High", status: "Active", health: 88, nextMaintenance: "4500 km" },
]
    ...
    <div className="grid lg:grid-cols-12 gap-8 h-[calc(100vh-12rem)] min-h-[600px]">
       ...
       <Card className="lg:col-span-4 bg-slate-950/40 border-white/5 shadow-2xl rounded-[2.5rem] flex flex-col overflow-hidden">
          <div className="p-8 border-b border-white/5 bg-white/[0.02]">
             ...
             <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-2">{VEHICLES.length} unités connectées au réseau</p>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
             {VEHICLES.map((v) => (
                <div 
                  key={v.id}
                  onClick={() => { setSelectedTruck(v); setSelectedPart(null) }}
                  className={cn(
                     "p-4 rounded-2xl border transition-all cursor-pointer group hover:bg-white/5",
                     selectedTruck.id === v.id ? "bg-white/10 border-emerald-500/50" : "bg-transparent border-white/5"
                  )}
                >
                   <div className="flex justify-between items-start mb-2">
                      <div>
                         <span className={cn(
                            ...
                            selectedTruck.id === v.id ? "text-white" : "text-slate-400 group-hover:text-white"
                         )}>{v.id}</span>
                         <p className="text-[10px] font-bold text-slate-500 uppercase">{v.model}</p>
                      </div>
                      <Badge className={cn("border-none text-[9px] font-black uppercase", v.status === "Critical" ? "bg-red-500/10 text-red-500" : "bg-emerald-500/10 text-emerald-500")}>
                         {v.status}
                      </Badge>
                   </div>
                   <div className="space-y-1">
                      ...
                      <Progress value={v.health} className={cn("h-1 bg-slate-800", v.health < 50 ? "text-red-500" : "text-emerald-500")} />
                   </div>
                </div>
             ))}
          </div>
          ...
          <div className="absolute top-6 right-6 z-10 flex gap-4">
             <div className="bg-black/40 backdrop-blur-md border border-white/10 p-4 rounded-2xl text-right">
                ...
                <div className="flex items-center gap-2 justify-end">
                   {selectedTruck.status === "Critical" && <AlertTriangle size={14} className="text-red-500 animate-pulse" />}
                   <p className={cn("text-xl font-black", selectedTruck.status === "Critical" ? "text-red-500" : "text-white")}>
             ...
             <div className="absolute bottom-6 left-6 right-6 lg:left-auto lg:w-80 bg-slate-950/90 backdrop-blur-xl border border-white/10 p-6 rounded-3xl shadow-2xl animate-in slide-in-from-bottom-10 fade-in duration-300">
                <div className="flex justify-between items-start mb-4">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         ...
                         <p className="text-[10px] text-slate-500 font-bold uppercase">Diagnostic Télémétrique</p>
                      </div>
                   </div>
                   <Button variant="ghost" size="icon" className="h-6 w-6 text-slate-500 hover:text-white" onClick={() => setSelectedPart(null)}>
                      X
                   </Button>
                </div>
                ...
                <div className="space-y-4">
                   {selectedPart.includes("Roue") ? (
                      ...
                      <>
                         <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Température</span>
                            <span className="text-sm font-black text-white">92°C</span>
                         </div>
                         <div className="flex justify-between items-center p-3 bg-white/5 rounded-xl">
                            <span className="text-[10px] text-slate-400 font-bold uppercase">Vibrations</span>
       ...
       <div className="lg:col-span-12 grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-slate-950/40 border-white/5 p-6 flex items-center gap-4 group hover:bg-white/5 transition-all">
             ...
             <div>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Meilleur Chauffeur</p>
          ...
          <Card className="bg-slate-950/40 border-white/5 p-6 flex items-center gap-4 group hover:bg-white/5 transition-all">
             ...
             <div>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Eco-Conduite</p>
          ...
          <Card className="bg-slate-950/40 border-white/5 p-6 flex items-center gap-4 group hover:bg-white/5 transition-all">
             ...
             <div>
                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest">Missions Complétées</p>
