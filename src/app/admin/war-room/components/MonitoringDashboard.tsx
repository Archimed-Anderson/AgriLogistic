...
import { Activity, Cpu, ShieldCheck, Clock, Zap } from 'lucide-react';
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
      ...
      <MetricCard
        ...
        color={
          metrics.cpu > 80 ? 'text-rose-500' : metrics.cpu > 60 ? 'text-amber-500' : 'text-primary'
        }
      ...
      <div className="bg-card/40 backdrop-blur-md border border-border p-5 rounded-2xl flex items-center justify-between group transition-all hover:bg-card/60 shadow-lg">
        <div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-black font-mono tracking-tight text-foreground transition-all">
              {metrics.timestamp}
            </span>
          </div>
        </div>
    ...
    >
      <div className="flex items-start justify-between mb-2">
        <div>
          ...
          <h4
            className={cn(
              'text-xl font-black tracking-tighter group-hover:scale-105 origin-left transition-transform',
              color
            )}
          >
            {value}
        ...
        <Icon
          className={cn('w-5 h-5 opacity-40 group-hover:opacity-100 transition-opacity', color)}
        />
      </div>
