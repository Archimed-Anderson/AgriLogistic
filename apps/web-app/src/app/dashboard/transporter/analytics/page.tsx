  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          <h1 className="text-5xl font-black text-white uppercase tracking-tighter">ANALYTICS <span className="text-emerald-500">PRECISION</span></h1>
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.4em] mt-2">Intelligence de Marché & Performance Opérationnelle</p>
        </div>
        <div className="flex gap-4">
           <Button variant="outline" className="border-white/5 bg-slate-900/50 text-slate-400 rounded-xl h-12 flex gap-2 font-black uppercase text-[10px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
         {[
           ...
           <Card key={i} className="bg-slate-950/40 border-white/5 shadow-xl rounded-3xl p-8 space-y-4">
              <div className="flex justify-between items-start">
                 <div className="h-10 w-10 rounded-xl bg-white/5 flex items-center justify-center text-slate-400">
                    <stat.icon size={20} className={stat.trend === 'up' ? 'text-emerald-500' : stat.trend === 'down' ? 'text-orange-500' : 'text-slate-400'} />
                 </div>
                 <Badge className="bg-white/5 text-slate-500 border-none font-black text-[9px] uppercase">{stat.trend}</Badge>
              </div>
              <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
         <Card className="lg:col-span-8 bg-slate-950/40 border-white/5 shadow-2xl rounded-[2.5rem] p-8 space-y-8">
            <div className="flex items-center justify-between">
               <div className="space-y-1">
                  <h3 className="text-xl font-black text-white uppercase tracking-tighter">Efficacité vs Coûts</h3>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Optimisation hebdomadaire par le moteur OR-Tools</p>
               </div>
            ...
            <div className="h-[400px] w-full mt-4">
               <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={performanceData}>
                     ...
                     <Tooltip 
                        contentStyle={{ backgroundColor: '#020617', border: '1px solid rgba(255,255,255,0.05)', borderRadius: '12px' }}
         ...
         <Card className="lg:col-span-4 bg-slate-950/40 border-white/5 shadow-2xl rounded-[2.5rem] p-8 flex flex-col items-center justify-center text-center space-y-8">
            <div className="space-y-1 w-full text-left">
               <h3 className="text-xl font-black text-white uppercase tracking-tighter">Segmentation Flux</h3>
               <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest">Répartition sectorielle des revenus</p>
            </div>
