...
export function ActivityTimeline({ activities }: { activities: Activity[] }) {
  ...
  const getColor = (type: Activity['type']) => {
    switch (type) {
      case 'order': return 'text-emerald-500 bg-emerald-500/10 border-emerald-500/20';
      case 'registration': return 'text-primary bg-primary/10 border-primary/20';
  ...
  return (
    <div className="flex flex-col h-full bg-card/40 backdrop-blur-xl border border-border rounded-[40px] overflow-hidden shadow-2xl transition-all hover:bg-card/50">
      <div className="p-8 border-b border-border bg-foreground/5 flex items-center justify-between">
        <div>
          <h3 className="text-sm font-black text-foreground uppercase tracking-tighter">Flux d'Activité</h3>
          <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">Temps Réel • {activities.length} alertes</p>
        </div>
        ...
        <AnimatePresence initial={false}>
          {activities.map((activity) => {
            ...
            return (
              ...
              >
                <div className={cn("p-3 rounded-2xl border shrink-0 transition-transform group-hover:scale-110", style)}>
                  <Icon className="w-4 h-4" />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-0.5">
                    <p className="text-[11px] font-black text-foreground truncate uppercase tracking-tight">{activity.title}</p>
                    {activity.amount && (
                      <span className="text-[10px] font-black text-emerald-500 tabular-nums">{activity.amount}</span>
                    )}
                  </div>
                  <div className="flex items-center gap-2">
                    <span className="text-[9px] font-bold text-muted-foreground uppercase">{activity.user}</span>
                    <div className="w-1 h-1 rounded-full bg-border" />
                    <span className="text-[9px] font-mono text-muted-foreground font-bold">{activity.timestamp}</span>
                  </div>
                </div>
