  ...
  <div className="kpi-card-premium bg-black/40 backdrop-blur-md border border-white/5 p-6 rounded-2xl flex flex-col gap-2">
    <div className="flex justify-between items-start">
      ...
      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
        ...
        color === 'magenta' ? 'bg-pink-500/10 text-pink-400' :
          ...
          <AreaChart data={data}>
            <defs>
              <linearGradient id={`grad-${color}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%" stopColor={color === 'cyan' ? '#00f2ff' : color === 'green' ? '#00ff41' : '#ff00d9'} stopOpacity={0.3}/>
                <stop offset="95%" stopColor={color === 'cyan' ? '#00f2ff' : color === 'green' ? '#00ff41' : '#ff00d9'} stopOpacity={0}/>
              </linearGradient>
            </defs>
            <Area 
              type="monotone" 
...
const StrategicKPIs: React.FC = () => {
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
      <KPICard 
        label="Volume en Transit" 
        value="1,450 t" 
        trend="+12.5%" 
        color="cyan" 
      />
      <KPICard 
        label="Gains Plateforme" 
        value="8.2M FCFA" 
        trend="+8.2%" 
        color="green" 
      />
