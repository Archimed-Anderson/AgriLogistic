...
const MOCK_USERS: User[] = [
  ...
  },
  {
    id: 'USR-002',
    name: 'Sophie Ndiaye',
    email: 'sophie@agri.com',
    role: 'buyer',
    status: 'active',
    lastLogin: 'Il y a 15m',
  },
  {
    id: 'USR-003',
    name: 'Ibrahima Faye',
    email: 'ibra@logistics.sn',
    role: 'transporter',
    status: 'active',
    lastLogin: 'Hier',
  },
    ...
    <div className="flex flex-col h-full bg-card/40 backdrop-blur-xl rounded-[40px] overflow-hidden border border-border shadow-2xl">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">
                {MOCK_USERS.length} Comptes Identifiés
              </p>
            </div>
          </div>
          <Button
        ...
        <table className="w-full text-left border-separate border-spacing-y-2">
          ...
          <tbody>
            {filteredUsers.map((user) => (
              <tr
                key={user.id}
                className="group hover:bg-foreground/5 transition-all duration-300 rounded-[24px]"
              >
                ...
                <td className="py-4 text-right pr-4 rounded-r-[24px]">
                  <div className="flex items-center justify-end gap-2">
                    ...
                    <Button
                      variant="ghost"
                      size="icon"
                      className="w-9 h-9 rounded-xl hover:bg-rose-500/10 text-rose-500"
                    >
      ...
      <div className="p-6 bg-foreground/5 border-t border-border flex items-center justify-between font-mono">
        <p className="text-[9px] font-black text-muted-foreground/60 uppercase tracking-[0.2em]">
          Affichage de {filteredUsers.length} Entrées
        </p>
