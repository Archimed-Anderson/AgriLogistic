...
export function EquipmentCard({ equipment, onClick }: EquipmentCardProps) {
  ...
  const getTypeBadge = () => {
    if (equipment.type === 'LOCATION') {
      return { bg: 'bg-blue-100', text: 'text-blue-700', border: 'border-blue-300', label: 'Location' }
    }
    if (equipment.type === 'VENTE') {
      return { bg: 'bg-emerald-100', text: 'text-emerald-700', border: 'border-emerald-300', label: 'Vente' }
    }
    return { bg: 'bg-purple-100', text: 'text-purple-700', border: 'border-purple-300', label: 'Location & Vente' }
  }
  ...
  const getPriceDisplay = () => {
    ...
    if (equipment.price) {
      return (
        <div className="flex items-baseline gap-1">
          <span className="text-2xl font-black text-emerald-600">{equipment.price.toLocaleString()}€</span>
        </div>
      )
    }
    return null
  ...
  return (
    ...
    >
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        {/* Type badge */}
        <div className={`absolute top-4 right-4 px-3 py-1.5 rounded-xl ${typeBadge.bg} ${typeBadge.text} border-2 ${typeBadge.border} backdrop-blur-sm`}>
      ...
      <div className="p-6">
        ...
        {/* Availability */}
        <div className={`px-4 py-2 rounded-xl ${availabilityBadge.bg} ${availabilityBadge.text} text-center`}>
