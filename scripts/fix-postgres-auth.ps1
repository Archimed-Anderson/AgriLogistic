...
try {
    docker exec AgriLogistic-postgres sh -c "sed -i 's/host    all             all             127.0.0.1\/32            trust/host    all             all             127.0.0.1\/32            scram-sha-256/' /var/lib/postgresql/data/pg_hba.conf && sed -i 's/host    all             all             ::1\/128                 trust/host    all             all             ::1\/128                 scram-sha-256/' /var/lib/postgresql/data/pg_hba.conf" | Out-Null
...
try {
    $result = docker exec AgriLogistic-postgres psql -U AgriLogistic -c "SELECT pg_reload_conf();" | Select-String -Pattern "t"
    if ($result) {
