                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-6">
                  <div className="flex items-center gap-6 text-[11px] font-bold text-slate-400 uppercase tracking-widest">
                    <span className="flex items-center gap-2.5">
                      <Calendar className="h-4 w-4 text-emerald-500" /> {post.publishedAt}
                    </span>
                    <span className="flex items-center gap-2.5">
                      <Clock className="h-4 w-4 text-emerald-500" /> {post.readTime}
                    </span>
                  </div>
            ...
            <article className="w-full lg:w-[65%] order-2 lg:order-1">
              ...
              >
                ...
                <div className="my-20">
                  <blockquote className="relative p-12 rounded-[50px] bg-[#0A2619] text-white overflow-hidden">
                    ...
                    <span className="text-6xl text-emerald-400 font-serif absolute top-4 left-8 opacity-20">
                      "
                    </span>
                    <p className="text-2xl md:text-3xl font-black leading-tight relative z-10 m-0">
                      La technologie ne remplace pas l'agriculteur, elle lui donne les
                      super-pouvoirs nécessaires pour faire face au changement climatique.
                    </p>
              ...
              <div className="mt-20 pt-12 border-t border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-8">
                <div className="flex flex-wrap gap-2.5">
                  {['AgTech', 'IA', 'Innovation'].map((tag) => (
                    <span
                      key={tag}
                      className="px-5 py-2.5 bg-slate-50 rounded-xl text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary hover:bg-emerald-50 transition-all cursor-pointer"
                    >
                      #{tag}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-6">
                  ...
                  <div className="flex items-center gap-3">
                    <button className="p-3 bg-slate-50 rounded-full hover:bg-primary hover:text-white transition-all">
                      <Share2 className="h-4 w-4" />
                    </button>
                  </div>
                </div>
              ...
              <div className="mt-16 p-8 md:p-12 rounded-[48px] bg-slate-50 border border-slate-100 flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="text-center md:text-left space-y-3">
                  <h4 className="text-xl font-black text-[#0A2619]">
                    Écrit par {post.author.name}
                  </h4>
                  <p className="text-sm text-slate-500 leading-relaxed">
                    Expert reconnu en {post.category.toLowerCase()} avec plus de 10 ans d'expérience
                    dans l'accompagnement des transformations digitales agricoles.
                  </p>
            ...
            <aside className="w-full lg:w-[30%] order-1 lg:order-2">
              <div className="sticky top-32 space-y-12">
                ...
                <div className="p-10 rounded-[48px] bg-[#0A2619] text-white relative overflow-hidden shadow-2xl">
                  ...
                  <p className="text-emerald-50/60 font-medium text-sm mb-8">
                    Les meilleures analyses AgTech chaque lundi.
                  </p>
                    ...
                    {relatedPosts.map((p) => (
                      <Link key={p.slug} href={`/blog/${p.slug}`} className="group flex gap-5">
                        ...
                        <div className="space-y-1.5">
                          <h4 className="text-sm font-black text-primary leading-snug group-hover:text-emerald-500 transition-colors line-clamp-2">
                            {p.title}
                          </h4>
                ...
                <Link
                  href="/blog"
                  className="flex items-center justify-center gap-2 w-full py-4 rounded-2xl border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:text-primary hover:bg-slate-50 transition-all group"
                >
