...
test.describe('Accessibilité - Dashboard de Connexion', () => {
  ...
  test('devrait être navigable entièrement au clavier', async ({ page }) => {
    ...
    // Naviguer à travers tous les éléments interactifs
  ...
  test('devrait avoir des textes alternatifs pour les icônes', async ({ page }) => {
    // Vérifier que les icônes SVG ont aria-hidden="true" ou aria-label
    ...
    for (let i = 0; i < Math.min(iconCount, 5); i++) {
      ...
      // L'icône doit avoir soit aria-hidden="true" soit aria-label
  ...
  test('devrait être compatible avec les lecteurs d\'écran', async ({ page }) => {
    ...
    await expect(submitButton).toHaveText(/se connecter/i)
  })
