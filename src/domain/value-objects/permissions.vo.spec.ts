...
describe('Permissions Value Object', () => {
  ...
  describe('Permission checks', () => {
    ...
    it('should check if has any of multiple permissions', () => {
      expect(
        farmerPermissions.hasAny(Permission.CREATE_PRODUCT, Permission.DELETE_ANY_PRODUCT)
      ).toBe(true);

      expect(
        farmerPermissions.hasAny(Permission.DELETE_ANY_PRODUCT, Permission.MANAGE_PLATFORM)
      ).toBe(false);
    });

    it('should check if has all of multiple permissions', () => {
      expect(
        farmerPermissions.hasAll(Permission.CREATE_PRODUCT, Permission.VIEW_ALL_PRODUCTS)
      ).toBe(true);

      expect(
        farmerPermissions.hasAll(Permission.CREATE_PRODUCT, Permission.DELETE_ANY_PRODUCT)
      ).toBe(false);
    });
  });
