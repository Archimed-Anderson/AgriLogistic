  ...
  loading: () => (
    <div className="w-full h-full flex items-center justify-center bg-slate-950 text-slate-500">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="font-mono text-sm uppercase tracking-widest text-emerald-500">Connexion Satellite...</p>
      </div>
    </div>
  ),
})
...
export default function CommandCenterPage() {
  ...
  return (
    <div className="relative w-full h-[calc(100vh-64px)] overflow-hidden bg-slate-950 font-sans">
      ...
      <Link href="/" className="absolute top-4 left-16 md:top-6 md:left-6 z-50 flex items-center gap-3 transition-transform hover:scale-105 active:scale-95 group pointer-events-auto">
        ...
        <div className="glass px-6 py-2.5 rounded-full flex items-center gap-6 pointer-events-auto border border-emerald-500/20 bg-slate-900/80 backdrop-blur-xl shadow-[0_0_20px_rgba(16,185,129,0.15)] transition-all hover:bg-slate-900/90 group">
          ...
          <div className="flex items-center gap-4 text-xs font-medium text-slate-300">
             <div className="flex items-center gap-2">
                ...
                <span className="group-hover:text-white transition-colors">{activeLoads.length} Chargements</span>
             </div>
             <div className="flex items-center gap-2">
                <Truck className="w-3.5 h-3.5 text-blue-400" />
                <span className="group-hover:text-white transition-colors">{availableTrucks.length} Camions</span>
             </div>
          </div>
        </div>
        ...
        <div className="absolute bottom-20 right-80 z-50 w-80 animate-in fade-in slide-in-from-bottom-5 duration-300">
           <div className="glass rounded-2xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl overflow-hidden flex flex-col h-96">
              ...
              <div className="p-4 bg-gradient-to-r from-blue-600/20 to-purple-600/20 border-b border-white/5 flex justify-between items-center">
                ...
                <Button size="icon" variant="ghost" className="h-6 w-6 text-slate-400 hover:text-white" onClick={() => setChatOpen(false)}>
                  <X className="w-4 h-4" />
                </Button>
              </div>
                 ...
                 <div className="space-y-4">
                    <div className="flex gap-2">
                       ...
                       <div className="bg-white/10 rounded-2xl rounded-tl-none p-3 text-xs text-slate-300 max-w-[85%]">
                          Bonjour ! Je suis l'assistant IA. Vous êtes en relation directe pour négocier le transport.
                       </div>
                    </div>
                    <div className="flex gap-2">
                       <div className="w-6 h-6 rounded-full bg-blue-900/50 flex-shrink-0 flex items-center justify-center text-[10px] text-blue-400">CH</div>
                       <div className="bg-blue-600/20 border border-blue-500/20 rounded-2xl rounded-tl-none p-3 text-xs text-blue-100 max-w-[85%]">
                          Salut, je suis disponible sur le secteur avec un camion vide. Je peux prendre votre chargement dans 2h.
                       </div>
                    </div>
                 </div>
              </ScrollArea>
      ...
      <div className="absolute top-4 left-4 z-20 md:hidden">
        <Button size="icon" variant="secondary" onClick={() => setLeftSidebarOpen(!leftSidebarOpen)}>
          {leftSidebarOpen ? <X className="w-4 h-4" /> : <Package className="w-4 h-4" />}
        </Button>
      </div>
      <div className="absolute top-4 right-4 z-20 md:hidden">
        <Button size="icon" variant="secondary" onClick={() => setRightSidebarOpen(!rightSidebarOpen)}>
          ...
          <div className="p-5 border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-red-400 text-[10px] font-bold uppercase tracking-wider">Live Feed</span>
              </div>
            </div>
          ...
          <ScrollArea className="flex-1 p-3">
            <div className="space-y-3">
              {activeLoads.slice(0, 10).map(load => (
                <div key={load.id} className="relative group p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-red-500/30 transition-all duration-300 cursor-pointer overflow-hidden">
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        <span className="text-sm font-bold text-slate-100 block mb-0.5">{load.productType}</span>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex flex-col gap-2 mb-4">
                      ...
                      <div className="relative pl-3 border-l border-slate-700 space-y-3 py-1">
                         <div className="relative">
                            ...
                            <p className="text-xs text-slate-300 font-medium leading-none">{load.originCity}</p>
                         </div>
                         <div className="relative">
                            <div className="absolute -left-[16.5px] top-1 w-2 h-2 rounded-full bg-orange-500 shadow-[0_0_8px_rgba(249,115,22,0.5)]"></div>
                            <p className="text-xs text-slate-300 font-medium leading-none">{load.destinationCity}</p>
                         </div>
                      </div>
                    </div>
          ...
          <div className="p-5 border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-2">
                 ...
                 <span className="text-blue-400 text-[10px] font-bold uppercase tracking-wider">{availableTrucks.length} Dispo</span>
              </div>
            </div>
          </div>
          ...
          <ScrollArea className="flex-1 p-3">
            <div className="space-y-3">
              {availableTrucks.slice(0, 10).map(truck => (
                <div key={truck.id} className="group p-4 rounded-2xl bg-white/5 hover:bg-white/10 border border-white/5 hover:border-blue-500/30 transition-all duration-300 cursor-pointer">
                  <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="grid grid-cols-2 gap-2 mb-4">
                     <div className="bg-slate-950/50 p-2 rounded-xl border border-white/5 text-center">
                        <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-0.5">Capacité</div>
                        <div className="text-xs font-black text-white">{truck.capacity}T</div>
                     </div>
                     <div className="bg-slate-950/50 p-2 rounded-xl border border-white/5 text-center">
                        <div className="text-[9px] text-slate-500 uppercase tracking-wider mb-0.5">Avis</div>
