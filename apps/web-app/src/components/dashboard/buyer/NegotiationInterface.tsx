...
const MESSAGES = [
  {
    id: 1,
    sender: 'Coopérative Vallée',
    text: 'Bonjour, nous pouvons fournir les 50T de Maïs Grade A.',
    time: '10:30',
    type: 'received',
  },
  {
    id: 2,
    sender: 'Me',
    text: 'Le prix de 245$/T est un peu au-dessus du marché (238$).',
    time: '10:32',
    type: 'sent',
  },
  {
    id: 3,
    sender: 'Coopérative Vallée',
    text: "Nous garantissons une livraison en 48h et un taux d'humidité <12%.",
    time: '10:33',
    type: 'received',
  },
];
...
export function NegotiationInterface() {
  ...
  const handleSendMessage = () => {
    ...
    setMessages([
      ...messages,
      { id: Date.now(), sender: 'Me', text: input, time: 'Now', type: 'sent' },
    ]);
    setInput('');
  };
  ...
  return (
    <div className="grid grid-cols-12 gap-6 h-[750px]">
      ...
      <div
        className={`col-span-12 lg:col-span-8 flex flex-col gap-4 transition-all duration-500 ${showContract ? 'lg:col-span-4' : ''}`}
      >
        ...
        <Card className="bg-slate-900/50 border-slate-800 shrink-0">
          <div className="flex items-center overflow-x-auto p-2 gap-4 custom-scrollbar">
            ...
            {OFFERS.map((offer) => (
              <div
                key={offer.id}
                className="flex items-center gap-3 px-3 py-1 bg-slate-800/50 rounded border border-slate-700/50 min-w-fit"
              >
          ...
          <CardHeader className="py-3 px-4 border-b border-slate-800 bg-slate-900/80 flex flex-row justify-between items-center">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="flex items-center gap-1 text-[10px] text-emerald-500">
                  <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" /> En
                  ligne
                </span>
              </div>
            </div>
            {!showContract && (
              <Button
                ...
                className="bg-blue-600 hover:bg-blue-500 text-xs gap-2"
              >
      ...
      <div
        className={`col-span-12 lg:col-span-4 flex flex-col gap-4 transition-all duration-500 ${showContract ? 'lg:col-span-8' : ''}`}
      >
              ...
              <CardContent className="p-5 space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-slate-400 italic">
                    "Le vendeur a un stock périssable dans 72h. Il acceptera probablement une baisse
                    de prix contre un paiement rapide."
                  </p>
                </div>

                {/* Suggestion Card */}
                <div
                  className="p-4 rounded-xl bg-purple-500/10 border border-purple-500/20 relative group hover:bg-purple-500/20 transition-colors cursor-pointer"
                ...
                >
                  ...
                  <p className="text-sm text-slate-300 mb-3">"{aiSuggestion.text}"</p>
