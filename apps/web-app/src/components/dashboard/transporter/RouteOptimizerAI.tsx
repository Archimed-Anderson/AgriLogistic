...
const LOCATIONS = [
  ...
  { id: 3, name: "Rufisque Centre", lat: 14.7160, lng: -17.2721, type: "delivery" },
  { id: 4, name: "Diamniadio Pole", lat: 14.7258, lng: -17.1853, type: "delivery" },
  { id: 5, name: "Ferme Touba (Returns)", lat: 14.7400, lng: -17.1500, type: "return" },
]
    ...
    <div className="grid lg:grid-cols-12 gap-6 h-[800px]">
      ...
      <div className="lg:col-span-4 flex flex-col gap-4">
        <Card className="bg-slate-950/50 border-white/5 shadow-2xl rounded-3xl overflow-hidden flex-1">
           <div className="p-6 border-b border-white/5 bg-slate-900/50">
              ...
              <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Définissez vos arrêts pour le calcul</p>
           </div>
           <CardContent className="p-6 space-y-4 overflow-y-auto max-h-[400px]">
              {LOCATIONS.filter(l => l.type !== 'return').map((loc, i) => (
                 <div key={loc.id} className="flex items-center gap-3 p-3 bg-white/5 rounded-xl border border-white/5">
                    ...
                    <div className="flex-1">
                       ...
                       <p className="text-[10px] text-slate-500 font-mono">{loc.lat.toFixed(4)}, {loc.lng.toFixed(4)}</p>
                    </div>
                 </div>
              ))}
              <Button variant="outline" className="w-full border-dashed border-white/10 text-slate-400 hover:text-white hover:bg-white/5 h-12 rounded-xl text-xs uppercase font-bold">
           ...
           <div className="p-6 bg-slate-900/50 border-t border-white/5 mt-auto">
              ...
              >
                 {isOptimized ? <><Sparkles /> Itinéraire Optimisé</> : <><Zap /> Lancer Optimisation AI</>}
              </Button>
           </div>
        </Card>
           ...
           {isOptimized && (
             <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="grid grid-cols-2 gap-4">
                <Card className="bg-emerald-500/10 border-emerald-500/20 rounded-2xl p-4">
                   <p className="text-[9px] font-black text-emerald-400 uppercase tracking-widest mb-1">Gain Distance</p>
                   <p className="text-2xl font-black text-white">-45 km</p>
                </Card>
                <Card className="bg-orange-500/10 border-orange-500/20 rounded-2xl p-4">
                   <p className="text-[9px] font-black text-orange-400 uppercase tracking-widest mb-1">Carburant</p>
      ...
      <div className="lg:col-span-8 relative rounded-[2.5rem] overflow-hidden border border-white/5 shadow-3xl bg-slate-950">
         ...
         >
            ...
            {!isOptimized && (
               <Polyline positions={UNOPTIMIZED_ROUTE} pathOptions={{ color: '#ef4444', weight: 4, opacity: 0.6, dashArray: '10' }} />
            )}
                  ...
                  <Card className="bg-slate-950/90 backdrop-blur-xl border-emerald-500/50 shadow-2xl rounded-3xl overflow-hidden ring-1 ring-emerald-500/20">
                     <div className="bg-gradient-to-r from-emerald-600 to-emerald-800 p-4 flex justify-between items-center">
                        <div className="flex items-center gap-2 text-white">
                           <Package size={18} />
                           <span className="font-black uppercase text-xs tracking-widest">Opportunité Pooling</span>
                        </div>
                        <Badge className="bg-white text-emerald-700 font-bold text-[9px]">+ 15.000 FCFA</Badge>
                     </div>
                     <CardContent className="p-6 space-y-4">
                        <div>
                           <h4 className="font-bold text-white text-lg">3t Riz à ramener</h4>
                           <p className="text-xs text-slate-400">Disponible à <span className="text-emerald-400 font-bold">Ferme Touba</span></p>
                        </div>
                        ...
                        <div className="flex items-center justify-between text-xs text-slate-300 bg-white/5 p-3 rounded-xl">
                           ...
                           <div className="flex flex-col text-right">
                              <span className="uppercase text-[9px] text-slate-500 font-bold">Marge Nette</span>
