...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      if (token && storedRole) {
        setState({
          user: storedUser
            ...
            : { id: 'dev-id', email: 'dev@AgriLogistic.com', role: storedRole },
  ...
  const forgotPassword = React.useCallback(async (data: { email: string }) => {
    setState((prev) => ({ ...prev, isLoading: true, error: null }));
    try {
      // // DEV ONLY: Simulation de latence (Optimisé de 800ms à 300ms)
  ...
  /**
   * DEV ONLY: Accès rapide sans authentification réelle.
   * Le rôle doit être UserRole (format Backend : Admin, Farmer, Buyer, Transporter) — cohérent avec les boutons Accès Rapide.
   */
  const devLogin = React.useCallback(
    (role: UserRole) => {
      setState((prev) => ({ ...prev, isLoading: true }));

      // Mock token et stockage du rôle au format Backend (UserRole)
      ...
      // Délai configurable via NEXT_PUBLIC_DEV_LOGIN_DELAY_MS (défaut 250 ms, voir README Configuration Auth)
      const delay = Number(process.env.NEXT_PUBLIC_DEV_LOGIN_DELAY_MS) || 250;
  ...
  return (
    <AuthContext.Provider
      value={{ ...state, login, register, forgotPassword, devLogin, logout, clearError }}
    >
      {children}
    </AuthContext.Provider>
  );
