    ...
    <header className="h-16 border-b border-border bg-card/40 backdrop-blur-xl sticky top-0 z-50 px-4 md:px-6 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
         <div className="flex items-center gap-3">
            ...
            <div>
               <h1 className="text-sm font-black tracking-tight text-foreground uppercase leading-none">AgroLogistic</h1>
               <p className="text-[9px] font-bold text-muted-foreground uppercase tracking-widest">Admin Console</p>
            </div>
         </div>
      ...
      <div className="flex items-center gap-4">
         ...
         <DropdownMenu>
            <DropdownMenuTrigger asChild>
               <Button variant="ghost" className="pl-2 pr-4 py-1 h-auto rounded-full hover:bg-foreground/5 gap-3">
                  ...
                  <div className="text-left hidden md:block">
                     <p className="text-xs font-black text-foreground">{user?.name || 'Administrateur'}</p>
                     <p className="text-[9px] font-medium text-muted-foreground uppercase">{user?.roles?.[0] || 'Super Admin'}</p>
                  </div>
               </Button>
            </DropdownMenuTrigger>
            <DropdownMenuContent align="end" className="w-56 rounded-xl p-2">
               <DropdownMenuItem className="rounded-lg text-xs font-medium cursor-pointer text-rose-500 focus:bg-rose-500/10 focus:text-rose-600" onClick={logout}>
