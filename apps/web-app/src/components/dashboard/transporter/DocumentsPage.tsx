      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-bold uppercase text-[10px] tracking-[0.4em]">
            Coffre-fort Numérique & Conformité
          </p>
        </div>
          ...
          {docs.map((doc) => (
            ...
            >
              <Card className="bg-slate-950/40 border-white/5 hover:bg-white/5 transition-all duration-300 group overflow-hidden">
                <div className="p-4 flex items-center gap-6">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3">
                      ...
                      <Badge
                        variant={doc.status === 'Valid' ? 'default' : 'destructive'}
                        className={`text-[9px] uppercase font-black border-none ${
                          doc.status === 'Valid'
                    ...
                    <div className="flex items-center gap-4 text-slate-500 text-xs mt-1">
                      <span className="flex items-center gap-1 font-mono">
                        <Calendar size={12} /> Exp: {doc.expiry}
                      </span>
                ...
                {doc.status === 'Expired' && <div className="h-1 w-full bg-red-500" />}
              </Card>
            </motion.div>
          ))}
        </AnimatePresence>
