  ...
  const features = [
    {
      ...
      description: "Visualize your crop health, water usage, and yield predictions in one centralized dashboard using AI-driven data.",
      stats: [
    ...
    },
    {
      ...
      description: "Connect directly with verified buyers worldwide. Bypass middlemen and secure better prices for your produce with smart contracts.",
      stats: [
    ...
    },
    {
      ...
      description: "Track shipments in real-time, optimize routes automatically, and ensure cold-chain integrity from farm to table.",
      stats: [
    ...
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="bg-white border-b border-slate-100 py-4 px-6 md:px-12 flex justify-between items-center sticky top-0 z-50">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => onNavigate?.("/")}>
             ...
             <span className="font-bold text-xl tracking-tight text-slate-900 hidden sm:block">AgroLogistic</span>
        </div>
        <Button variant="ghost" className="text-slate-600 hover:text-green-700 hover:bg-green-50" onClick={() => onNavigate?.("/")}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <div className="inline-flex items-center rounded-full border border-green-200 bg-white px-3 py-1 shadow-sm mb-6">
              ...
              <span className="text-xs font-semibold text-green-700 uppercase tracking-wide">Interactive Demo Environment</span>
            </div>
            <h1 className="text-4xl font-extrabold text-slate-900 sm:text-5xl mb-6">
              Experience the Power of <br/>
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-green-600 to-emerald-500">AgroLogistic Enterprise</span>
            </h1>
            <p className="text-lg text-slate-600">
              Explore our platform's capabilities in a live simulation. See how our integrated tools transform agricultural operations.
            </p>
          </div>
          ...
          <div className="bg-white rounded-[2rem] shadow-2xl border border-slate-200 overflow-hidden ring-1 ring-slate-900/5">
             <div className="grid lg:grid-cols-12 min-h-[600px]">
                ...
                <div className="lg:col-span-3 bg-slate-50 border-r border-slate-100 p-6 flex flex-col gap-2">
                   <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-4 px-2">Select Module</p>
                   {features.map((feature) => (
                      <button
                        key={feature.id}
                        onClick={() => { setAnimate(false); setTimeout(() => setActiveFeature(feature.id), 50); }}
                        className={`w-full text-left px-4 py-4 rounded-xl flex items-center gap-3 transition-all duration-300 ${activeFeature === feature.id ? 'bg-white shadow-md border border-green-100 text-green-700' : 'text-slate-500 hover:bg-white/50 hover:text-slate-700'}`}
                      >
                         <div className={`p-2 rounded-lg ${activeFeature === feature.id ? 'bg-green-100 text-green-700' : 'bg-slate-200 text-slate-500'}`}>
                   ...
                   <div className="mt-auto bg-green-900 rounded-xl p-4 text-white relative overflow-hidden group cursor-pointer" onClick={() => onNavigate?.('/register')}>
                      ...
                      <Button size="sm" className="w-full bg-white text-green-900 hover:bg-green-50 border-none">
                         View Pricing
                      </Button>
                   </div>
                </div>
                ...
                <div className="lg:col-span-9 p-8 lg:p-12 relative bg-white">
                   <div className={`transition-all duration-500 ease-out transform ${animate ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'}`}>
                      ...
                      <div className="flex flex-col lg:flex-row gap-12 items-center">
                         <div className="flex-1 space-y-6">
                            ...
                            <p className="text-slate-600 text-lg leading-relaxed">{activeData.description}</p>
                            ...
                            <div className="pt-6">
                               <Button className="bg-green-600 hover:bg-green-700 text-white rounded-full px-8 h-12 gap-2" onClick={() => onNavigate?.('/register')}>
                                  Start Free Trial
      ...
      <section className="py-24 bg-white border-t border-slate-100">
         <div className="max-w-7xl mx-auto px-6 lg:px-8">
            <h2 className="text-3xl font-bold text-center text-slate-900 mb-16">Trusted by Industry Leaders</h2>
            <div className="grid md:grid-cols-3 gap-8">
               {[1, 2, 3].map((i) => (
                  <div key={i} className="bg-slate-50 p-8 rounded-2xl border border-slate-100 relative">
                     ...
                     <p className="text-slate-600 mb-6 italic">"This platform has completely revolutionized how we manage our supply chain. The analytics are game-changing."</p>
