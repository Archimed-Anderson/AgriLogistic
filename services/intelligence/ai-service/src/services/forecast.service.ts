...
import { InternalServiceClient } from './internal.client';
...
export class ForecastService {
  ...
  static async forecastDemand(productId: string, days = 30): Promise<ForecastResult[]> {
    ...
    try {
      // Get historical sales data from Order Service
      const history = await InternalServiceClient.getSalesHistory(productId, days);
  ...
  static async forecastPrice(productId: string, days = 14): Promise<ForecastResult[]> {
    ...
    try {
      // Get price history from Product Service
      const history = await InternalServiceClient.getPriceHistory(productId, days);
      
      if (history.length < 5) {
        // Get current price from Product Service
        const currentPrice = await InternalServiceClient.getCurrentProductPrice(productId);
